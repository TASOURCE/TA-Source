...
function bool CheckVehicle(UTBot B)
{
	...
	if (Vehicle(B.Pawn) == None && V != None && V.Health > 0 && !V.bDeleteMe)
	{
		if (V.PlayerStartTime > WorldInfo.TimeSeconds)
		{
			...
			{
				if ( (PC.PlayerReplicationInfo.Team == Team) && (PC.Pawn != None) && ((Vehicle(PC.Pawn) == None) )) //TGFIXUP || (UTVehicle_Hoverboard(PC.Pawn) != None)) )
				{
					bSkip = true;
					break;
				}
...
defaultproperties
{
   ...
   ObjectArchetype=UDKSquadAI'UDKBase.Default__UDKSquadAI'
}
