...
//$$ML: Always force a respawn (TODO: Remove the property from the options string!)
    /*
    ...
    */
	...
	//GoalScore = 3;//(!IsAutomatedPerfTesting()) ? Max(0,GetIntOption( Options, "GoalScore", GoalScore )) : 0;
    GoalScore = (!IsAutomatedPerfTesting()) ? Max(0,GetIntOption( Options, "GoalScore", GoalScore )) : 0;

	InOpt = ParseOption( Options, "Console");
	if ( (InOpt ~= "true") || WorldInfo.IsConsoleBuild() )
	{
...
//$$ML: BZ 8340. This was causing standalone games to scale damage incorrectly.
/*
...
function float RatePlayerStart(PlayerStart P, byte Team, Controller Player)
{
	...
	if (Player != None)
	{
		ForEach WorldInfo.AllControllers(class'Controller', OtherPlayer)
		{
			...
			{
                //$$ML: This was preventing players from spawning on starts that were encroaching on a spawned player.
                /*
...
defaultproperties
{
   ...
   GameSpecificMapCycles(0)=(GameClassName="UTVehicleCTFGame_Content",Maps=("TrCTF-Necropolis","TrCTF-SandStorm"))
   ...
   ObjectArchetype=UDKGame'UDKBase.Default__UDKGame'
}
