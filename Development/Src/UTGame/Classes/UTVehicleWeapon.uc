...
class UTVehicleWeapon extends UTWeapon
	abstract
	dependson(UTVehicle)
	native; //$$BE: Added native flag so TrVehicleWeapon can compile as native.
...
simulated function WeaponPlaySound( SoundCue Sound, optional float NoiseLoudness, optional bool bStopWhenOwnerDestroyed )
{
	...
	if(bPlaySoundFromSocket && MyVehicle != none && MyVehicle.Mesh != none)
	{
		...
		Instigator.PlaySound(Sound, false, true,bStopWhenOwnerDestroyed,Loc);
	}
	else
...
defaultproperties
{
   DefaultImpactEffect=(DurationOfDecal=24.000000,DecalDissolveParamName="DissolveAmount")
   DefaultAltImpactEffect=(DurationOfDecal=24.000000,DecalDissolveParamName="DissolveAmount")
