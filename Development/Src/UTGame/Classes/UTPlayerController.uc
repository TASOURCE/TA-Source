...
function AcknowledgePossession(Pawn P)
{
	...
	if ( LocalPlayer(Player) != None )
	{
			...
			&& (IdentifiedTeam != PlayerReplicationInfo.Team.TeamIndex) )
		{
			...
			if ( IdentifiedTeam < 2 )
			{
                //$$ML: IdentifyTeamMessage() so we can override it in Tribes.
...
//$$ML: Change to IdentifyTeamMessage() so we can override it in Tribes.
...
state Dead
{
	...
	{
        //$$ JCR - Don't respawn automatically anymore
        return;
...
simulated event GetPlayerViewPoint( out vector POVLocation, out Rotator POVRotation )
{
	...
	{
		if ( PlayerCamera != None && !IsSpectating() ) //$$
		{
			PlayerCamera.Destroy();
			PlayerCamera = None;
		}
		...
		if ( ViewTarget != None )
		{
			...
			{
				//$$ML: Ability to override camera function for watching other pawns.
				//ViewTarget.CalcCamera( DeltaTime, POVLocation, POVRotation, FOVAngle );
				CalcCameraOnViewTarget(DeltaTime, POVLocation, POVRotation, FOVAngle);
				//$$ML: End.
			}
			//$$ML: We roll our own Freecamera.
			/*if ( bFreeCamera )
			{
				POVRotation = Rotation;
			}
...
function SendMessage(PlayerReplicationInfo Recipient, name MessageType, float Wait, optional class<DamageType> DamageType)
{
	...
	if( UTPlayerReplicationInfo(PlayerReplicationInfo).VoiceClass != none )
	{
...
//$$ML: The score for a team has changed.
...
defaultproperties
{
   ...
   Begin Object Class=CylinderComponent Name=CollisionCylinder Archetype=CylinderComponent'UDKBase.Default__UDKPlayerController:CollisionCylinder'
      ReplacementPrimitive=None
      Name="CollisionCylinder"
      ObjectArchetype=CylinderComponent'UDKBase.Default__UDKPlayerController:CollisionCylinder'
   End Object
   ...
   ObjectArchetype=UDKPlayerController'UDKBase.Default__UDKPlayerController'
}
