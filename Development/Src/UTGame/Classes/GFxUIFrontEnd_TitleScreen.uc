...
function OnCloseAnimationComplete()
{
    ...
    switch (Selection)
    {
        case(MENU_BTN_CAMPAIGN):
            UT_ConsoleCommand("open "$CampaignMap$"?game=TribesGame.TrGame", true);
            break;
        case(MENU_BTN_INSTANTACTION):
            UT_ConsoleCommand("open "$InstantActionMap $ "?game=TribesGame.TrGame", true);
            break;
        case(MENU_BTN_EXIT):
...
defaultproperties
{
   ...
   Name="Default__GFxUIFrontEnd_TitleScreen"
