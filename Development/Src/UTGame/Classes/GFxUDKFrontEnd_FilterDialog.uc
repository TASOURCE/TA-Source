...
function OnViewLoaded()
{
	...
	SearchDataStore = UTDataStore_GameSearchDM( class'UIRoot'.static.StaticResolveDataStore( 'TrGameSearch' ) );
}
...
{
	if ( FriendlyName ~= "Rabbit" )
	{
		return "TribesGame.TrGame_TRRabbit";
	}
	else if ( FriendlyName ~= "Deathmatch" )
	{
		return "TribesGame.TrGame_TRTDM";
	}

    return "TribesGame.TrGame_TRCTF";
}
...
defaultproperties
{
   ListOptions(0)=(OptionName="Type",OptionLabel="MATCH TYPE",OptionData=("LAN","Internet"))
   ListOptions(1)=(OptionName="Mode",OptionLabel="GAME MODE",OptionData=("Capture the Flag","Team Deathmatch","Rabbit"))
   Name="Default__GFxUDKFrontEnd_FilterDialog"
   ObjectArchetype=GFxUDKFrontEnd_Dialog'UTGame.Default__GFxUDKFrontEnd_Dialog'
}
