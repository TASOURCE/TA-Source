...
var() editinline ForceFeedbackWaveform WeaponFireWaveForm;
 ...
 */
//$$BE: Added optional flag for stopping when owner destroyed
simulated function WeaponPlaySound(SoundCue Sound, optional float NoiseLoudness, optional bool bStopWhenOwnerDestroyed)
{
	...
	{
		Instigator.PlaySound(Sound, false, true, bStopWhenOwnerDestroyed);
	}
}
...
simulated function TimeWeaponEquipping()
{
	...
	//$$ML: Commented this out because it also gets called in UTPawn::WeaponChanged().
	// Play the animation
	//PlayWeaponEquip(); 
...
simulated function DetachWeapon()
{
	...
	if (P != None)
	{
		...
		{
			//$$BE Commented out, caused weird 3P device anims
...
simulated event SetPosition(UDKPawn Holder, float DeltaSeconds)
{
	...
	if ( Holder.Controller == None )
	{
		if ( DemoRecSpectator(PC) != None )
		{
			...
			SpecRotation.Yaw = LagRot(SpecRotation.Yaw & 65535, LastRotation.Yaw & 65535, MaxYawLag, 0, DeltaSeconds);
			SpecRotation.Pitch = LagRot(SpecRotation.Pitch & 65535, LastRotation.Pitch & 65535, MaxPitchLag, 1, DeltaSeconds);
 ...
 * @param PC - cast of Instigator.Controller for convenience
 */
//$$BE Added a new bool for determining if we are returning to 3P
...
simulated state Active
{
    ...
    //$$ML: Was causing problems with the Global version in derived classes.
    /*
...
//$$BE
/** called when Instigator exits a vehicle while we are its Weapon */
...
defaultproperties
{
   ...
   ObjectArchetype=UDKWeapon'UDKBase.Default__UDKWeapon'
}
