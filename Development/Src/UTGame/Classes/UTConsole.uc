...
//$$ START - JCR - Block input while browsers are open
native function bool AnyBrowserOpen();
//$$ END
...
function bool InputKey( int ControllerId, name Key, EInputEvent Event, float AmountDepressed = 1.f, bool bGamepad = FALSE )
{
	...
	if ( AnyBrowserOpen() || (ConsoleTargetPlayer != None && ConsoleTargetPlayer.Actor.WorldInfo.IsInSeamlessTravel() ))
	{
		return false;
	}
...
function OutputTextLine(coerce string Text, optional Color OverrideColor) //$$ PC 
{
	TextCount++;
	Super.OutputTextLine(text, OverrideColor);  //$$
}
