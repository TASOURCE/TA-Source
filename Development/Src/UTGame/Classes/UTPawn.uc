...
var() editinline class<UTFamilyInfo> CurrCharClassInfo;
...
//$$ML:Class for client side weapon pawn.
var class<UTClientSideWeaponPawn> m_ClientSideWeaponPawnClass;
...
function DropFlag(optional Controller C)
{
	local UTPlayerReplicationInfo UTPRI;
...
simulated function PlayDying(class<DamageType> DamageType, vector HitLoc)
{
	...
	ForEach LocalPlayerControllers(class'PlayerController', PC)
	{
		if( pc.ViewTarget == self )
		{
			if (UTHudBase(pc.MyHud)!=none)
				UTHudBase(pc.MyHud).DisplayHit(HitLoc, 100, DamageType);
		...
		if (DrivenWeaponPawn.BaseVehicle != LastDrivenWeaponPawn.BaseVehicle || DrivenWeaponPawn.SeatIndex != LastDrivenWeaponPawn.SeatIndex)
		{
			if (DrivenWeaponPawn.BaseVehicle != None)
			{
				...
				if (ClientSideWeaponPawn == None || ClientSideWeaponPawn.bDeleteMe)
				{
                    ...
                    ClientSideWeaponPawn = Spawn(m_ClientSideWeaponPawnClass, DrivenWeaponPawn.BaseVehicle);
				}
...
{
	//$$BE Added check to make sure Mesh isn't none
	if ((CurrentWeaponAttachment == None || CurrentWeaponAttachment.Class != CurrentWeaponAttachmentClass) && Mesh != none && Mesh.SkeletalMesh != None)
	{
...
function PlayHit(float Damage, Controller InstigatedBy, vector HitLocation, class<DamageType> damageType, vector Momentum, TraceHitInfo HitInfo)
{
	...
	if (WorldInfo.TimeSeconds - LastPainSound >= MinTimeBetweenPainSounds)
	{
		...
		if (Damage > 0 && Health > 0)
		{
			...
			{
				//$$ML: Consider adding this back in, but for now it is causing logspam. SoundGroupClass.static.PlayTakeHitSound(self, Damage);
			}
		}
	}
	...
	if (Damage > 0 || (Controller != None && Controller.bGodMode))
	{
		...
		//$$BE Did we damage ourself?
		LastTakeHitInfo.m_bDamagedSelf = InstigatedBy == Controller;
...
simulated function bool IsLocationOnHead(const out ImpactInfo Impact, float AdditionalScale)
{
	...
	//$$BE: Fixed headshots not working properly in dedicated server.
	if (WorldInfo.NetMode == NM_DedicatedServer && DrivenWeaponPawn.BaseVehicle == none)
	{
		HeadLocation.Z += BaseTranslationOffset;
	}
...
defaultproperties
{
   ...
   TauntNames(4)="TauntE"
   TauntNames(5)="TauntF"
   CamOffset=(X=4.000000,Y=16.000000,Z=-13.000000)
   m_ClientSideWeaponPawnClass=Class'UTGame.UTClientSideWeaponPawn'
   bCanDoubleJump=True
   ...
   Begin Object Class=CylinderComponent Name=CollisionCylinder Archetype=CylinderComponent'UDKBase.Default__UDKPawn:CollisionCylinder'
      ...
      ObjectArchetype=CylinderComponent'UDKBase.Default__UDKPawn:CollisionCylinder'
   End Object
   ...
   Begin Object Class=ArrowComponent Name=Arrow Archetype=ArrowComponent'UDKBase.Default__UDKPawn:Arrow'
      ArrowColor=(B=255,G=200,R=150,A=255)
      bTreatAsASprite=True
      ReplacementPrimitive=None
      Name="Arrow"
      ObjectArchetype=ArrowComponent'UDKBase.Default__UDKPawn:Arrow'
   End Object
   Components(1)=Arrow
   ...
   ObjectArchetype=UDKPawn'UDKBase.Default__UDKPawn'
}
