...
//$$BE Changed this to 255.
const UTVEHICLE_UNSET_TEAM = 255;
...
//$$BE Changed to a static array.
var PlayerReplicationInfo PassengerPRI[4];
...
//$$BE
/** The Gib part that the death camera follows */
var UTGib DeathCameraGib;
	...
	for(i=0;i<VehicleSounds.Length;++i)
	{
		...
		if(VehicleSounds[i].SoundStartTag == SoundTag)
		{
			if(VehicleSounds[i].SoundRef == none)
			{
				...
				//$$ML: Added bShouldRemainActiveIfDropped so that we don't lose track of this sound.
				VehicleSounds[i].SoundRef.bShouldRemainActiveIfDropped = true;
				//$$ML: End.
			}
				...
				class<DamageType> DamageType, float Momentum, vector HitLocation, Actor DamageCauser, optional float DamageFalloffExponent=1.f)
{
	...
	{
		//$$BE: Added variable 'P' and applying damage to directly-attached pawns.
		...
		{
			P.TakeRadiusDamage(EventInstigator, DamageAmount, DamageRadius, DamageType, Momentum, HitLocation, false, DamageCauser, DamageFalloffExponent);
		}
	}
...
function controller GetControllerForSeatIndex(int SeatIndex)
{
	...
	else if (Seats[SeatIndex].StoragePawn != none)
		return Seats[SeatIndex].StoragePawn.Controller;
...
simulated function PlayerReplicationInfo GetSeatPRI(int SeatNum)
{
	...
	{
		//$$BE PassengerPRI is an array now.
...
{
	local float Dist, xscale, MaxOffset;
		...
		screenLoc.Y >= Canvas.ClipY)
	{
		...
		if ( !bShowLocked && !bShowUseable && !bPlayingSpawnEffect && ((UTPawn(PC.Pawn) != None)))//TGFIXUP || (UTVehicle_Hoverboard(PC.Pawn) != None)) )
		{
			bShowUseable = ShouldShowUseable(PC, Dist);
		}
		if ( !bShowUseable )
	...
	if ( !bPlayingSpawnEffect && (UTPawn(PC.Pawn) != None))//TGFIXUP || (UTVehicle_Hoverboard(PC.Pawn) != None)) )
	{
...
{
	//$$BE PassengerPRI is an array now.
	if ( PassengerPRI[0] != None )
	{
		PostRenderPassengerBeacon(PC, Canvas, TeamColor, TextColor, Weap, PassengerPRI[0], PassengerTeamBeaconOffset);
	}
}
...
function PlayHorn()
{
	...
	if (PlayerController(Controller) != None)
	{
		...
		for (i = 0; i < Seats.length; i++)
		{
			//$$BE Fixed log spam on honking horn.
			if (Seats[i].SeatPawn != none && Seats[i].SeatPawn.Controller == None)
			{
				NumPositions++;
			}
...
//$$BE: Added SeatIndex to parameters.
...
function bool DriverEnter(Pawn P)
{
	...
	//$$BE Added index to function.
	HandleEnteringFlag(UTPlayerReplicationInfo(PlayerReplicationInfo), 0);
...
{	
	...
	//$$BE Added index
	HandleEnteringFlag(UTPlayerReplicationInfo(Seats[SeatIndex].SeatPawn.PlayerReplicationInfo), SeatIndex);
...
event bool DriverLeave(bool bForceLeave)
{
	...
	if (bResult)
	{
		...
		//$$ML: This line below was commented out with no comment... but we need to remove the storagepawn if the driver leaves.
...
simulated function vector GetCameraStart(int SeatIndex)
{
	...
	//$$BE Use the death camera gib for determining location.
	if (DeathCameraGib != none)
		return DeathCameraGib.Location;
...
{
	//$$BE: It really wants SeatSocket here, because it's using it for positioning.
	...
	{
		UTP.SetBase( Self, , Mesh, Seats[SeatIndex].SeatSocket);
	}
	else
	{
		UTP.SetBase( Self );
...
function SetSeatStoragePawn(int SeatIndex, Pawn PawnToSit)
{
	...
	if ( (SeatIndex == 1) && (Role == ROLE_Authority) )
	{
		if (PawnToSit == none)
			...
			PassengerPRI[0] = PawnToSit.PlayerReplicationInfo;
		else
			PassengerPRI[0] = Seats[SeatIndex].SeatPawn.PlayerReplicationInfo;
	}
...
defaultproperties
{
   ...
   bCanFlip=False
   Begin Object Class=RB_StayUprightSetup Name=MyStayUprightSetup Archetype=RB_StayUprightSetup'UDKBase.Default__UDKVehicle:MyStayUprightSetup'
      Name="MyStayUprightSetup"
      ObjectArchetype=RB_StayUprightSetup'UDKBase.Default__UDKVehicle:MyStayUprightSetup'
   End Object
   StayUprightConstraintSetup=RB_StayUprightSetup'UTGame.Default__UTVehicle:MyStayUprightSetup'
   Begin Object Class=RB_ConstraintInstance Name=MyStayUprightConstraintInstance Archetype=RB_ConstraintInstance'UDKBase.Default__UDKVehicle:MyStayUprightConstraintInstance'
      Name="MyStayUprightConstraintInstance"
      ObjectArchetype=RB_ConstraintInstance'UDKBase.Default__UDKVehicle:MyStayUprightConstraintInstance'
   End Object
   ...
   EyeHeight=30.000000
   Begin Object Class=SkeletalMeshComponent Name=SVehicleMesh Archetype=SkeletalMeshComponent'UDKBase.Default__UDKVehicle:SVehicleMesh'
      ...
      Name="SVehicleMesh"
      ObjectArchetype=SkeletalMeshComponent'UDKBase.Default__UDKVehicle:SVehicleMesh'
   End Object
   Mesh=SVehicleMesh
   Begin Object Class=CylinderComponent Name=CollisionCylinder Archetype=CylinderComponent'UDKBase.Default__UDKVehicle:CollisionCylinder'
      ...
      ObjectArchetype=CylinderComponent'UDKBase.Default__UDKVehicle:CollisionCylinder'
   End Object
   ...
   ObjectArchetype=UDKVehicle'UDKBase.Default__UDKVehicle'
}
