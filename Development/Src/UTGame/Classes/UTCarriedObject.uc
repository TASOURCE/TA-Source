...
function SetHolder(Controller C)
{
	...
	//$$ML: moved this here after Holder is set so we can broadcast appropriately.
	LogTaken(c);
...
function Score()
{
	...
	if ( !WorldInfo.Game.bGameEnded && HomeBase != none)
	{
		Disable('Touch');
...
event Drop(optional Controller Killer, optional bool bNoThrow)
{
	...
	if (HomeBase != none) HomeBase.ObjectiveChanged();
...
event Landed(vector HitNormal, actor FloorActor)
{
	...
	//$$ML: Removed rotation changes for the flag.
...
auto state Home
{
	...
	function Timer()
	{
        ...
        if (HomeBase != none) FinalLoc = HomeBase.Location + (HomeBaseOffset >> HomeBase.Rotation);
	...
	function BeginState(Name PreviousStateName)
	{
		...
		Disable('Touch');
		bHome = true;
		if (HomeBase != none && SetLocation(HomeBase.Location + (HomeBaseOffset >> HomeBase.Rotation)))
		{
			SetRotation(HomeBase.Rotation);
...
defaultproperties
{
   ...
   ObjectArchetype=UDKCarriedObject'UDKBase.Default__UDKCarriedObject'
}
