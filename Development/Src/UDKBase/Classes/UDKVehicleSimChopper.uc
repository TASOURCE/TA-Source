...
//$$BE Moved AnglePID here so we can use it in our sim base classes.
struct native AnglePID
{
	var float PGain, DGain, IGain;
	var float PrevIntegral, PrevError;

	structcpptext
	{
		FAnglePID()
		{
			PrevIntegral = PrevError = 0.0f;
		}

		FLOAT Update(FLOAT DesiredAngle, INT CurrentRotatorAngle, FLOAT DeltaTime);
	}
};
...
defaultproperties
{
   ...
   Name="Default__UDKVehicleSimChopper"
