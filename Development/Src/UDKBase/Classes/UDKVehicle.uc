...
//$$BE
/** The VehicleSeatAnims struct defines the animtree and animset for the seat when using a given armor type. */
struct native VehicleSeatAnimInfo
{
	var() bool      bIsArmorDependent;
	var() bool      bIsFactionDependent;
	var() bool      bClearAnimSet;
	var() int       ArmorType;
	var() int       TeamType;
    var() AnimSet   SeatAnimSet;
	var() AnimTree  SeatAnimTree;
};
...
struct native VehicleSeat
{
	...
	//$$BE Zoom Settings
	var() name CameraZoomTag;
	var() vector CameraZoomBaseOffset;
	var() float CameraZoomOffset;
	...
	//$$BE Vehicle Anim Seat Info
	var() array<VehicleSeatAnimInfo> m_SeatAnimInfo;
	var() bool      m_bAttachPawnDirectly;  // Only relevant for passengers. Should this seat just base the passenger to the vehicle rather than spawning a WeaponPawn?
	var() float     m_fViewYawLimit;	    // Only relevant for passengers, limits the amount of yaw off of the seat socket's X axis.
	var() Vector    m_vRidingPassengerMeshOffset; // Only relevant for riding passengers, amount to offset the translation on their mesh.
	var() vector    m_vIdealExitLocation;   // Offset from the SeatBone where the pawn leaving the seat will exit.
	
	var array<GameSkelCtrl_Recoil> m_TurretRecoilControls; // Handles recoiling when the vehicle fires.
	var() array<name> m_TurretRecoilControlNames;

	var() float m_fTurnInterpSpeed; // The speed it takes to ramp up/down the rotation.
    //$$ML: End.

	//$$BE:FlagPosition
	var() name FlagSocketName;      // The socket the flag will attach to for this seat.
	var() vector FlagOffset;        // The offset of the flag from the bone.
	var() rotator FlagRotation;     // The rotation of the flag from the bone.
	var() bool ValidFlagSeat;       // Whether the flag can be held in this seat.
	//$$BE: End.
...
//$$BE Added new function
/** Determine if this is happening local on the vehicle. */
simulated event bool IsLocalOnVehicle(optional Pawn InP)
{
	local int i;
	local Pawn P;
	local PlayerController PC;

	// Is this sound being played locally?
	if (InP == none)
	{
		for (i = 0; i < Seats.length; i++)
		{
			...
			if (P != none && PlayerController(P.Controller) != none)
			{
				...
				{
					if (PC != none && PlayerController(P.Controller) == PC)
					{
						return true;
					}
				}
	...
	{
		if (InP != none && PlayerController(InP.Controller) != none)
		{
			foreach LocalPlayerControllers(class'PlayerController', PC)
			{
				if (PC != none && PlayerController(InP.Controller) == PC)
				{
					return true;
				}
			}
...
defaultproperties
{
   ...
   End Object
   StayUprightConstraintSetup=RB_StayUprightSetup'UDKBase.Default__UDKVehicle:MyStayUprightSetup'
   ...
   End Object
   StayUprightConstraintInstance=RB_ConstraintInstance'UDKBase.Default__UDKVehicle:MyStayUprightConstraintInstance'
