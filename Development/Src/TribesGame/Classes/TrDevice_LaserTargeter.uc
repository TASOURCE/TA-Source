 ...
 * 
 */
class TrDevice_LaserTargeter extends TrDevice_ConstantFire
	dependson(TrObject)
	native;
...
var float m_fCallInEndTime;

var ParticleSystemComponent m_pscLaserEffect;

var repnotify TrCallIn r_CallIns[TR_MAX_CALLINS];

var byte r_ActiveCallin;

var byte m_PrevActiveCallin;

var SoundCue m_CallInConfirmation;

var rotator m_LastOwnerRot;
var vector m_LastTargetPos;

native function bool IsValidTargetLocation(vector CurrentTarget, vector PreviousTarget, optional Actor HitTarget);

replication
{
	if (bNetDirty && bNetOwner && Role == ROLE_Authority)
		r_CallIns, r_ActiveCallin;
}

simulated event ReplicatedEvent(name VarName)
{
    if (VarName == 'r_CallIns')
    {
        UpdateCallins();
    }
}
...
simulated function PostBeginPlay()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		// Are call-ins enabled?
		TrGRI = TrGameReplicationInfo(WorldInfo.GRI);
		if (TrGRI != none && TrGRI.r_ServerConfig != none)
		{
            if (TrGRI.r_ServerConfig.IsCallinAllowed(i))
            {
		        r_CallIns[i] = Spawn(class'TrCallIn_TacticalStrike', Owner);
                r_CallIns[i].Initialize(TrGRI.r_ServerConfig.CallinPrices[i],
                                        TrGRI.r_ServerConfig.CallinBuildups[i],
                                        TrGRI.r_ServerConfig.CallinCooldowns[i]);
            }

            if (TrGRI.r_ServerConfig.IsCallinAllowed(++i))
            {
		        r_CallIns[i] = Spawn(class'TrCallIn_SupportInventory', Owner);
                r_CallIns[i].Initialize(TrGRI.r_ServerConfig.CallinPrices[i],
                                        TrGRI.r_ServerConfig.CallinBuildups[i],
                                        TrGRI.r_ServerConfig.CallinCooldowns[i]);
            }
            ...
            if (TrGRI.r_ServerConfig.IsCallinAllowed(++i))
            {
		        r_CallIns[i] = Spawn(class'TrCallIn_OrbitalStrike', Owner);
                r_CallIns[i].Initialize(TrGRI.r_ServerConfig.CallinPrices[i],
                                        TrGRI.r_ServerConfig.CallinBuildups[i],
                                        TrGRI.r_ServerConfig.CallinCooldowns[i]);
            }
		...
		// Loop through all perks on the owner.
		if (Instigator != none)
		{
			for (i = 0; i < TR_MAX_CALLINS; i++)
			{
				if (r_CallIns[i] != none)
				{
					r_CallIns[i].Instigator = Instigator;
					r_CallIns[i].bNetDirty = true;
				}
			}
		}
	}
...
simulated function UpdateCallins()
{
	local int i;
	local TrGameReplicationInfo TrGRI;
...
simulated event Destroyed()
{
	...
	if (Role == ROLE_Authority)
	{
		for (i = 0; i < TR_MAX_CALLINS; i++)
		{
			// If the call-in is in-progress don't destroy it.
			if (r_CallIns[i] != none)
			{
				if (r_CallIns[i].IsInProgress)
				{
					r_CallIns[i].bPendingDelete = true;
					r_CallIns[i] = none;
				}
				else
				{
...
simulated function bool CanFireCallIn()
{
	local TrPlayerController TrPC;

	TrPC = TrPlayerController(Instigator.Controller);

	// Can we fire this call-in?
	if (r_ActiveCallin < 0 || r_ActiveCallin >= TR_MAX_CALLINS || 
		r_CallIns[r_ActiveCallin] == none || 
		TrPC == none ||
		(TrPC.m_fCallInCooldown[r_ActiveCallin] > 0.0 &&  TrPC.m_fCallInCooldown[r_ActiveCallin] + r_CallIns[r_ActiveCallin].CooldownTime > WorldInfo.TimeSeconds))
	{
		return false;
	}
...
simulated function bool IsValidCallIn(byte CallInIndex)
{
	if (CallInIndex < 0 || CallInIndex >= TR_MAX_CALLINS || r_CallIns[CallInIndex] == none)
	{
		return false;
	}
...
simulated function SetActiveCallIn(byte CallInIndex)
{
	// Replicate down to client.
	if (Role == ROLE_Authority)
	{
		bNetDirty = true;
	}
	...
	// Swap call-ins.
	r_ActiveCallin = CallInIndex;

	if (r_ActiveCallin != m_PrevActiveCallin)
	{
		UpdateCallInMaterial();
	}

	// Stop firing the current call-in.
...
simulated function OnStartConstantFire()
{
	...
	m_fCallInEndTime = WorldInfo.TimeSeconds + r_CallIns[r_ActiveCallin].BuildupTime;
}
...
simulated state WeaponConstantFiring
{
	...
	{
		local bool bHasEnoughCredits;
		local bool bNeedPower;
		local TrPlayerController TrPC;

		// Invalid active call-in.
		if (r_ActiveCallin < 0 || r_ActiveCallin >= TR_MAX_CALLINS)
		{
			EndFire(CurrentFireMode);
		...
		bHasEnoughCredits = r_CallIns[r_ActiveCallin].VerifyCredits();
        bNeedPower = r_CallIns[r_ActiveCallin].VerifyPower();
		...
		if (Role == ROLE_Authority)
		{
			...
			if( TrPC != none )
			{
                if (!bHasEnoughCredits)
                {
				    ...
				    TrPC.ReceiveLocalizedMessage(class'TrWarningMessage', 6,,,r_CallIns[r_ActiveCallin]);
                }
                else if (!bNeedPower)
                {
			        PlaySound(m_DryFireSoundCue);
				    TrPC.ReceiveLocalizedMessage(class'TrWarningMessage', 7,,,r_CallIns[r_ActiveCallin]);
                }
			}
		}

		// Invalid active call-in, or not enough credits.
		if (!bHasEnoughCredits)
		{
			EndFire(CurrentFireMode);
			GotoState(m_PostFireState);
	...
	simulated event Tick(float DeltaTime)
	{
		...
		UpdateCreditMaterial();
		UpdateLaserEffect(DeltaTime);
	}
}
...
simulated function bool GetLaserStartAndEnd(out Vector StartLocation, out Vector EndLocation, out Vector EndLocationNormal)
{
	local Array<ImpactInfo>	ImpactList;
	local ImpactInfo TestImpact;
	local Vector StartTrace, EndTrace;

	// Calculate the start position, aim direction, and end of the trace for the laser.
	StartTrace = InstantFireStartTrace();
	EndTrace = InstantFireEndTrace(StartTrace);

	// Set the default hit location normal to straight up.
	EndLocationNormal = vect(0,0,1);

	// Perform a trace and find the real end point.
	TestImpact = CalcWeaponFire(StartTrace, EndTrace, ImpactList);

	// If we hit nothing, then go to the end position.
	if (TestImpact.HitLocation != StartTrace || TestImpact.HitLocation != EndTrace)
	{
	...
	return TestImpact.HitActor != none && IsValidTargetLocation(EndLocation, m_LastTargetPos, TestImpact.HitActor) && CanFireCallIn();
}
...
simulated function SpawnLaserEffect()
{
	local bool hasLaserHit;
	local MaterialInstanceConstant BeamMIC;
	local TrPawn POwner;
	local Vector Start, End, HitNormal;

	hasLaserHit = GetLaserStartAndEnd(Start, End, HitNormal);
	...
	// Spawn the laser effect for clients only.
	if (m_pscLaserEffect == none && POwner.Controller != none && POwner.Controller.IsLocalPlayerController())
	{
		m_pscLaserEffect = WorldInfo.MyEmitterPool.SpawnEmitterMeshAttachment(m_TracerBeamTemplate, SkeletalMeshComponent(Mesh), 'WSO_Emit_01', true,,);
		m_pscLaserEffect.SetDepthPriorityGroup(SDPG_World);
		m_pscLaserEffect.SetTickGroup(TG_EffectsUpdateWork);
		m_pscLaserEffect.SetVectorParameter('TracerEnd', End);

		BeamMIC = new (m_pscLaserEffect.Outer) class'MaterialInstanceConstant';
		BeamMIC.SetParent(MaterialInstanceConstant'WEP_LaserTargeter_1p.Effects.MIC_FX_Beam');
		BeamMIC.SetScalarParameterValue('bValidLocation', hasLaserHit ? 1.0f : 0.0f);
		m_pscLaserEffect.SetMaterialParameter('Beam_Material', BeamMIC);
	}

	UpdateTarget(hasLaserHit, End, 0.0f);
}

simulated function UpdateLaserEffect(float DeltaTime)
{
	local bool hasLaserHit;
	local MaterialInterface BeamMIOut;
	local MaterialInstanceConstant BeamMIC;
	local Vector Start, End, HitNormal;

	hasLaserHit = GetLaserStartAndEnd(Start, End, HitNormal);

	// Update the laser target end position.
	if (m_pscLaserEffect != none)
	{
		// Set the end position.
		m_pscLaserEffect.SetVectorParameter('TracerEnd', End);

		// Change the material for valid/invalid locations.
		if (m_pscLaserEffect.GetMaterialParameter('Beam_Material', BeamMIOut))
		{
			BeamMIC = MaterialInstanceConstant(BeamMIOut);
			if (BeamMIC != none)
			{
				BeamMIC.SetScalarParameterValue('bValidLocation', hasLaserHit ? 1.0f : 0.0f);
			}
	...
	UpdateTarget(hasLaserHit, End, DeltaTime);

	// Is it time to activate the call-in?
	if (m_fCallInEndTime > 0 && m_fCallInEndTime <= WorldInfo.TimeSeconds)
	{
		// Notify the server to perform the call-in.
		if (WorldInfo.NetMode != NM_DedicatedServer)
		{
			ServerPerformCallIn(End, HitNormal);
		}
...
reliable client function CallInConfirmed()
{
	if (m_CallInConfirmation != none)
	{
		PlaySound(m_CallInConfirmation, true);
	}
}

reliable server function ServerPerformCallIn(vector EndLocation, vector HitNormal)
{
    local TrPlayerController TrPC;

	if (r_CallIns[r_ActiveCallin] == none)
	{
		return;
	}

	// Perform verification on the server.
	if (m_fCallInEndTime > 0 && IsValidTargetLocation(EndLocation, m_LastTargetPos))
	{
		if (r_CallIns[r_ActiveCallin].FireCompletedCallIn(r_ActiveCallin, EndLocation, HitNormal))
		{
			// Notify the client the call-in has been confirmed.
			CallInConfirmed();
		}
        ...
        if (Instigator.Controller != none)
        {
            ...
            if (TrPC != none && TrPC.Stats != none)
            {
                TrPC.Stats.CallIn(TrPC);
            }
        }
...
simulated function UpdateTarget(bool hasHitSomething, Vector End, float DeltaTime)
{
	local TrPawn POwner;
	...
	if (hasHitSomething)
	{
		// Update the call-in end time.
		if (m_fCallInEndTime <= 0.0f)
		{
			m_fCallInEndTime = WorldInfo.TimeSeconds + r_CallIns[r_ActiveCallin].BuildupTime;
		}
	...
	// We haven't hit something, kill the end point.
	else
	{
		m_fCallInEndTime = FClamp(m_fCallInEndTime + DeltaTime * 2.0f, WorldInfo.TimeSeconds, WorldInfo.TimeSeconds + r_CallIns[r_ActiveCallin].BuildupTime);
		m_LastTargetPos = vect(0,0,0);
	}
}

// Used for calculating the HUD charge bar.
simulated function float CalcHUDAimChargePercent()
{
	// Invalid active call-in, or not enough credits.
	if (r_ActiveCallin < 0 || 
		r_ActiveCallin >= TR_MAX_CALLINS || 
		!r_CallIns[r_ActiveCallin].VerifyCredits() || 
		!r_CallIns[r_ActiveCallin].VerifyPower() || 
		m_fCallInEndTime <= 0)
	{
		return 0.0f;
	}

	if (m_fCallInEndTime <= WorldInfo.TimeSeconds)
	{
		return 1.0f;
	}

	return (r_CallIns[r_ActiveCallin].BuildupTime - (m_fCallInEndTime - WorldInfo.TimeSeconds)) / r_CallIns[r_ActiveCallin].BuildupTime;
}
...
/** called to display call-in cost */
simulated function int GetAmmoCount()
{
	...
	return r_CallIns[r_ActiveCallin].GetCreditCost();
}

simulated function UpdateCreditMaterial()
{
	local TrPawn POwner;
	...
	// Only set the material if it's valid.
	if (Mesh.Materials.Length > 1 && Mesh.Materials[1] != none)
	{
		MaterialInstanceConstant(Mesh.Materials[1]).SetScalarParameterValue('bAvailable', (r_CallIns[r_ActiveCallin].VerifyCredits() && r_CallIns[r_ActiveCallin].VerifyPower())? 1.0f : 0.0f);
		MaterialInstanceConstant(Mesh.Materials[1]).SetScalarParameterValue('bCoolingDown', CanFireCallIn() ? 0.0f : 1.0f);
	}

	// Update the "ammo display" to have the credit cost.
	m_nCarriedAmmo = GetAmmoCount();
}

/** Update the MIC on the mesh */
...
/** Update the MIC on the mesh */
simulated function UpdateCallInMaterial()
{
	local MaterialInstanceConstant DisplayMaterial;

	// Invalid active call-in.
	if (r_ActiveCallin < 0 || r_ActiveCallin >= TR_MAX_CALLINS || Mesh == none)
	{
		return;
	}

	m_PrevActiveCallin = r_ActiveCallin;

	// Create the new MIC.
	DisplayMaterial = new (Mesh.Outer) class'MaterialInstanceConstant';
	DisplayMaterial.SetParent(r_CallIns[r_ActiveCallin].MICDisplay);
	Mesh.SetMaterial(1, DisplayMaterial);
...
defaultproperties
{
   ...
   m_vZoomMeshOffset=(X=0.000000,Y=0.000000,Z=-30.000000)
   ...
   AttachmentClass=Class'TribesGame.TrAttachment_LaserTargeter'
   MuzzleFlashSocket=
   MuzzleFlashDuration=0.000000
   PlayerViewOffset=(X=7.000000,Y=2.000000,Z=-5.000000)
   SmallWeaponsOffset=(X=12.000000,Y=0.000000,Z=1.000000)
   WideScreenOffsetScaling=1.000000
   WidescreenRotationOffset=(Pitch=-100,Yaw=0,Roll=0)
   InstantHitDamageTypes(0)=Class'TribesGame.TrDmgType_LaserTargeter'
   InstantHitDamageTypes(1)=()
   FireOffset=(X=3.000000,Y=10.000000,Z=-7.000000)
   bInstantHit=True
   WeaponRange=262143.000000
   ...
   Name="Default__TrDevice_LaserTargeter"
