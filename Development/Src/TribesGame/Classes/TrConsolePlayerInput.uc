...
class TrConsolePlayerInput extends TrPlayerInput within TrConsolePlayerController
	config(Input);
...
function PreProcessInput( float DeltaTime )
{
	...
	if( Pawn == none )
	{
 	...
 	bUsingTiltController = TrConsolePlayerController(Pawn.Controller).IsControllerTiltActive();
...
defaultproperties
{
   ...
   ViewAccel_TimeToHoldBeforeFastAcceleration=0.125000
   ...
   bUsingGamepad=True
   Name="Default__TrConsolePlayerInput"
   ObjectArchetype=TrPlayerInput'TribesGame.Default__TrPlayerInput'
}
