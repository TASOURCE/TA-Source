/* Super class to implement buffs that have lifetime*/

class TrTimedPowerup extends TrInventory
	abstract;
...
simulated function DisplayPowerup(Canvas Canvas, HUD HUD, float ResolutionScale,out float YPos)
{
...
function bool DenyPickupQuery(class<Inventory> ItemClass, Actor Pickup)
{
	...
	if (ItemClass == Class)
	{
		...
		if (Drop != None && TrTimedPowerup(Drop.Inventory) != None)
		{
			TimeRemaining += TrTimedPowerup(Drop.Inventory).TimeRemaining;
		}
		else
		{
...
defaultproperties
{
   ...
   Begin Object Class=SpriteComponent Name=Sprite Archetype=SpriteComponent'TribesGame.Default__TrInventory:Sprite'
      ReplacementPrimitive=None
      HiddenGame=True
      AlwaysLoadOnClient=False
      AlwaysLoadOnServer=False
      Name="Sprite"
      ObjectArchetype=SpriteComponent'TribesGame.Default__TrInventory:Sprite'
   End Object
   Components(0)=Sprite
   Name="Default__TrTimedPowerup"
   ObjectArchetype=TrInventory'TribesGame.Default__TrInventory'
}
