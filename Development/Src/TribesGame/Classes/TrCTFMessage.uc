...
class TrCTFMessage extends UTLocalMessage
    	abstract;
...
// Array slots: 0=self, 1=teammate, 2=enemy
var SoundNodeWave m_ReturnSounds[3];
var SoundNodeWave m_DroppedSounds[3];
var SoundNodeWave m_TakenSounds[3];
var SoundNodeWave m_CapturedSounds[3];

var SoundCue m_sBEFlagTaken;
var SoundCue m_sBEFlagCaptured;
var SoundCue m_sDSFlagTaken;
var SoundCue m_sDSFlagCaptured;

var SoundCue m_sDSLostBEFlag;
var SoundCue m_sBELostDSFlag;

var SoundCue m_sDSFlagSecured;
var SoundCue m_sBEFlagSecured;
var SoundCue m_sDSTookBEFlag;
var SoundCue m_sBETookDSFlag;
var SoundCue m_sDSCapturedBEFlag;
var SoundCue m_sBECapturedDSFlag;

var(Message) localized string EnemyFlagTaken;
var(Message) localized string FriendlyFlagTaken;
var(Message) localized string EnemyFlagPickupAwayFromBase;
var(Message) localized string FriendlyFlagPickupAwayFromBase;
var(Message) localized string EnemyFlagCaptured;
var(Message) localized string FriendlyFlagCaptured;
var(Message) localized string EnemyFlagReturned;
var(Message) localized string FriendlyFlagReturned;
var(Message) localized string EnemyFlagDropped;
var(Message) localized string FriendlyFlagDropped;
var(Message) localized string EnemyFlagAbandoned;
var(Message) localized string FriendlyFlagAbandoned;
var(Message) localized string BloodEagleFlagTaken;
var(Message) localized string DiamondSwordFlagTaken;
var(Message) localized string BloodEagleFlagPickupAwayFromBase;
var(Message) localized string DiamondSwordFlagPickupAwayFromBase;
var(Message) localized string BloodEagleFlagCaptured;
var(Message) localized string DiamondSwordFlagCaptured;
var(Message) localized string BloodEagleFlagReturned;
var(Message) localized string DiamondSwordFlagReturned;
var(Message) localized string BloodEagleFlagDropped;
var(Message) localized string DiamondSwordFlagDropped;
var(Message) localized string BloodEagleFlagAbandoned;
var(Message) localized string DiamondSwordFlagAbandoned;

//
//	Switch is encoded - low bit is true for local team; rest of switch is the event code
//
	...
	)
{
	...
	local TrPlayerController SpectatorPC;

	messageString="";

	SpectatorPC = TrPlayerController(OptionalObject);
	if( SpectatorPC != none )
	{
		switch( Switch )
		{
		case 0:
			// BloodEagle flag has been taken from base.
			messageString = Default.BloodEagleFlagTaken;
			break;
		case 1:
			// DiamondSword flag has been taken from base.
			messageString = Default.DiamondSwordFlagTaken;
			break;
		case 2:
			// BloodEagle flag has been taken from being dropped.
			messageString = Default.BloodEagleFlagPickupAwayFromBase;
			break;
		case 3:
			// DiamondSword flag has been taken from being dropped.
			messageString = Default.DiamondSwordFlagPickupAwayFromBase;
			break;
		case 4:
			// BloodEagle flag has been captured.
			messageString = Default.BloodEagleFlagCaptured;
			break;
		case 5:
			// DiamondSword flag has been captured.
			messageString = Default.DiamondSwordFlagCaptured;
			break;
		case 6:
			// BloodEagle flag has been returned.
			if (RelatedPRI_1!=None)
			{
				messageString = Default.BloodEagleFlagReturned;
			}
			else
			{
				messageString = Default.BloodEagleFlagAbandoned;
			}
			break;
		case 7:
			// DiamondSword flag has been returned.
			if (RelatedPRI_1!=None)
			{
				messageString = Default.DiamondSwordFlagReturned;
			}
			else
			{
				messageString = Default.DiamondSwordFlagAbandoned;
			}
			break;
		case 8:
			// BloodEagle flag has been dropped.
			messageString = Default.BloodEagleFlagDropped;
			break;
		case 9:
			// DiamondSword flag has been dropped.
			messageString = Default.DiamondSwordFlagDropped;
			break;
		}

		if (RelatedPRI_1!=None)
		{
			messageString = RelatedPRI_1.PlayerName@messageString;
		}
	...
	else
	{
		...
		{
		case 0:
			// Enemy flag has been taken from base.
			messageString = Default.EnemyFlagTaken;
			break;
		case 1:
			// Friendly flag has been taken from base.
			messageString = Default.FriendlyFlagTaken;
			break;
		case 2:
			// enemy flag has been taken from being dropped.
			messageString = Default.EnemyFlagPickupAwayFromBase;
			break;
		case 3:
			// Friendly flag has been taken from being dropped.
			messageString = Default.FriendlyFlagPickupAwayFromBase;
			break;
		case 4:
			// Enemy flag has been captured.
			messageString = Default.EnemyFlagCaptured;
			break;
		case 5:
			// Friendly flag has been captured.
			messageString = Default.FriendlyFlagCaptured;
			break;
		case 6:
			// Enemy flag has been returned.
			if (RelatedPRI_1!=None)
			{
				messageString = Default.EnemyFlagReturned;
			}
			else
			{
				messageString = Default.EnemyFlagAbandoned;
			}
			break;
		case 7:
			// Friendly flag has been returned.
			if (RelatedPRI_1!=None)
			{
				messageString = Default.FriendlyFlagReturned;
			}
			else
			{
				messageString = Default.FriendlyFlagAbandoned;
			}
			break;
		case 8:
			// Enemy flag has been dropped.
			messageString = Default.EnemyFlagDropped;
			break;
		case 9:
			// Friendly flag has been dropped.
			messageString = Default.FriendlyFlagDropped;
			break;
		}
...
{
    local int FlagTeam, LocalPlayerTeam;
    local TeamInfo TI;
	//local bool bTouchedFlag;
    ...
    if( TI != None )
    {
        FlagTeam = TI.TeamIndex;
        LocalPlayerTeam = P.GetTeamNum();

        // The following music events play the stingers for team-based flag events (pickup, capture, drop, return, etc).
        switch( Switch )
        {
        case 0:
            if( FlagTeam != LocalPlayerTeam )
            {
                // Enemy flag has been taken from base.
                TrPlayerController(P).TrClientMusicEvent(11);
				sound = LocalPlayerTeam==0 ? Default.m_sBETookDSFlag : Default.m_sDSTookBEFlag;
            ...
            {
                // Friendly flag has been taken from base.
				sound = LocalPlayerTeam==0 ? Default.m_sBEFlagTaken : Default.m_sDSFlagTaken;
                TrPlayerController(P).TrClientMusicEvent(6);
            }
            break;
        case 1:
            if( FlagTeam != LocalPlayerTeam )
            {
                // Enemy flag has been taken from being dropped.
                TrPlayerController(P).TrClientMusicEvent(12);
            ...
            else
            {
                // Friendly flag has been taken from being dropped.
				//sound = LocalPlayerTeam==0 ? Default.m_sBEFlagTaken : Default.m_sDSFlagTaken;
                TrPlayerController(P).TrClientMusicEvent(7);
            }
            break;
        case 2:
			// Both flags are involved; we want FlagTeam to refer to the flag that was captured but it does not.  switch it:
			FlagTeam = (FlagTeam==1)?0:1;

            if( FlagTeam != LocalPlayerTeam )
            {
                // Enemy flag has been captured.
                TrPlayerController(P).TrClientMusicEvent(8);
				sound = LocalPlayerTeam==0 ? Default.m_sBECapturedDSFlag : Default.m_sDSCapturedBEFlag;
            }
            else
            {
                // Friendly flag has been captured.
                TrPlayerController(P).TrClientMusicEvent(13);
				sound = LocalPlayerTeam==0 ? Default.m_sBEFlagCaptured : Default.m_sDSFlagCaptured;
            }
            break;
        case 3:
            if( FlagTeam != LocalPlayerTeam )
            {
                // Enemy flag has been returned.
                TrPlayerController(P).TrClientMusicEvent(14);
				
				sound = LocalPlayerTeam==0 ? Default.m_sBELostDSFlag : Default.m_sDSLostBEFlag;
            }
            else
            {
                // Friendly flag has been returned.
                TrPlayerController(P).TrClientMusicEvent(9);
				sound = LocalPlayerTeam==0 ? Default.m_sBEFlagSecured : Default.m_sDSFlagSecured;
        ...
        case 4:
            ...
            {
                // Enemy flag has been dropped.
                TrPlayerController(P).TrClientMusicEvent(15);
            }
            else
            {
                // Friendly flag has been dropped.
                TrPlayerController(P).TrClientMusicEvent(10);
            }
            break;
        }
	}

	if( LocalPlayerTeam != 255 )
	{
		if (sound!=None)
	...
	if( LocalPlayerTeam == 255 )
	{
		Super.ClientReceive(P, Switch*2 + FlagTeam, RelatedPRI_1, RelatedPRI_2, P);
	}
	else
	{
		// encode the switch value with the team reference
		Super.ClientReceive(P, Switch*2+ (FlagTeam != LocalPlayerTeam?0:1), RelatedPRI_1, RelatedPRI_2, OptionalObject);
	}
}

defaultproperties
{
   EnemyFlagTaken="has the enemy flag"
   FriendlyFlagTaken="has our flag"
   EnemyFlagPickupAwayFromBase="has picked up the enemy flag"
   FriendlyFlagPickupAwayFromBase="has picked up our flag"
   EnemyFlagCaptured="has captured the enemy flag"
   FriendlyFlagCaptured="has captured our flag"
   EnemyFlagReturned="has returned the enemy flag"
   FriendlyFlagReturned="has returned our flag"
   EnemyFlagDropped="has dropped enemy flag"
   FriendlyFlagDropped="has dropped our flag"
   EnemyFlagAbandoned="The enemy flag has returned to base"
   FriendlyFlagAbandoned="Our flag has returned to base"
   BloodEagleFlagTaken="has the Blood Eagle flag"
   DiamondSwordFlagTaken="has the Diamond Sword flag"
   BloodEagleFlagPickupAwayFromBase="has picked up the Blood Eagle flag"
   DiamondSwordFlagPickupAwayFromBase="has picked up the Diamond Sword flag"
   BloodEagleFlagCaptured="has captured the Blood Eagle flag"
   DiamondSwordFlagCaptured="has captured the Diamond Sword flag"
   BloodEagleFlagReturned="has returned the Blood Eagle flag"
   DiamondSwordFlagReturned="has returned the Diamond Sword flag"
   BloodEagleFlagDropped="has dropped the Blood Eagle flag"
   DiamondSwordFlagDropped="has dropped the Diamond Sword flag"
   BloodEagleFlagAbandoned="The Blood Eagle flag has returned to base"
   DiamondSwordFlagAbandoned="The Diamond Sword flag has returned to base"
   MessageArea=4
   bIsUnique=True
   DrawColor=(B=255,G=160,R=0,A=255)
   FontSize=2
   Name="Default__TrCTFMessage"
