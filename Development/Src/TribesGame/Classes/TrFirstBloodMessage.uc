 ...
 * 
 */
class TrFirstBloodMessage extends UTLocalMessage;
...
var(Message) localized string FirstBlood;
	...
	)
{
	...
	TrPC = TrPlayerController(RelatedPRI_1.Owner);

	if( TrPC != none && TrPC.IsLocalPlayerController() )
	{
		return "You"@Default.FirstBlood;
	}

	return RelatedPRI_1.PlayerName@Default.FirstBlood;
}

defaultproperties
{
   FirstBlood="drew first blood"
   MessageArea=2
   bIsUnique=True
   Lifetime=6.000000
   DrawColor=(B=0,G=0,R=255,A=255)
   FontSize=2
   Name="Default__TrFirstBloodMessage"
