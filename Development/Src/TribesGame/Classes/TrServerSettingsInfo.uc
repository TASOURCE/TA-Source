class TrServerSettingsInfo extends Info
	native;
...
var int ArenaRounds;
var int RespawnTime;
var int OvertimeLimit;
var int TeamAssignType;
var int BaseDestructionLimit;
var int FFKillLimit;
var int FFDamageLimit;
var int SpawnType;

var bool bFriendlyFire;
var bool bAutoBalanceInGame;
var bool bObjectiveUpgrades;
var bool bPoweredDeployables;
var bool bGeneratorAutoRegen;
var bool bPreplacedObjectives;
var bool bGenDestroyable;
var bool bFriendlyFireBaseAssets;
var bool bFriendlyFireDeployables;
var bool bSupplyDropAmmoOnly;
var bool bPoweredCallins;
var bool bTeamCredits;

var int GameScores[GAME_TYPE.TGT_MAX];
var int ClassCounts[TrClassType.TCT_MaX];
var int VehicleLimits[EVehicleTypes.VEHICLE_MAX];
var int VehiclePrices[EVehicleTypes.VEHICLE_MAX];

var int CallinsDenied;
var int CallinPrices[TR_MAX_CALLINS];
var int CallinCooldowns[TR_MAX_CALLINS];
var float CallinBuildups[TR_MAX_CALLINS];
var float fFriendlyFireDamageMultiplier;
var float fVehicleHealthMultiplier;

var bool  bSkiEnabled;
var float fEnergyMultiplier;
var float fAoESizeMultiplier;
var float fAoEDamageMultiplier;
var int MaxSpeedWithFlagMedium;           // Units are KM/H
var int DecelerationRateWithFlag;         // Units are KM/H
var int MaxSpeedWithFlagLight;           // Units are KM/H
var int MaxSpeedWithFlagHeavy;           // Units are KM/H

var bool bCTFBlitzAllFlagsMove;

/** Events/functions for handling server properties */
native static function bool GetPropAsFloat (int PropId, out float  Value);
native static function bool GetPropAsInt   (int PropId, out int    Value);
native static function bool GetPropAsString(int PropId, out string Value);
native function             ApplyServerSettings();

event LoadServerSettings()
{
    local int IntVal;

	GetPropAsInt(GAME_CONFIG_TIMELIMIT, TimeLimit);
    GetPropAsInt(GAME_CONFIG_WARMUPTIME, WarmupTime);
	GetPropAsInt(GAME_CONFIG_MAXPLAYERS, MaxPlayers);
	GetPropAsInt(GAME_CONFIG_RESPAWNTIME, RespawnTime);
	GetPropAsInt(GAME_CONFIG_OVERTIMELIMIT, OvertimeLimit);
	GetPropAsInt(GAME_CONFIG_TEAMJOINTYPE, TeamAssignType);
	GetPropAsInt(GAME_CONFIG_BASEDESTLIMIT, BaseDestructionLimit);
	GetPropAsInt(GAME_CONFIG_ARENAROUNDS, ArenaRounds);
	GetPropAsInt(GAME_CONFIG_FFDMGLIMIT, FFDamageLimit);
	GetPropAsInt(GAME_CONFIG_FFKILLLIMIT, FFKillLimit);
	GetPropAsInt(GAME_CONFIG_SPAWNNAKED, SpawnType);
	GetPropAsInt(GAME_CONFIG_MAXSPEEDWITHFLAGLIGHT, MaxSpeedWithFlagLight);
	GetPropAsInt(GAME_CONFIG_DECELERATIONRATEWITHFLAG, DecelerationRateWithFlag);
	GetPropAsInt(GAME_CONFIG_MAXSPEEDWITHFLAGMEDIUM, MaxSpeedWithFlagMedium);
	GetPropAsInt(GAME_CONFIG_MAXSPEEDWITHFLAGHEAVY, MaxSpeedWithFlagHeavy);

	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_PTH, ClassCounts[TCT_PTH]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_SEN, ClassCounts[TCT_SEN]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_INF, ClassCounts[TCT_INF]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_SLD, ClassCounts[TCT_SLD]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_TCN, ClassCounts[TCT_TCN]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_RDR, ClassCounts[TCT_RDR]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_JUG, ClassCounts[TCT_JUG]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_DMB, ClassCounts[TCT_DMB]);
	GetPropAsInt(GAME_CONFIG_CLASSCOUNT_BRT, ClassCounts[TCT_BRT]);

	GetPropAsInt(GAME_CONFIG_SCORELIMIT_CTF, GameScores[TGT_CTF]);
	GetPropAsInt(GAME_CONFIG_SCORELIMIT_RAB, GameScores[TGT_RAB]);
	GetPropAsInt(GAME_CONFIG_SCORELIMIT_TDM, GameScores[TGT_TDM]);
	GetPropAsInt(GAME_CONFIG_SCORELIMIT_ARN, GameScores[TGT_ARN]);
	GetPropAsInt(GAME_CONFIG_SCORELIMIT_CAH, GameScores[TGT_CAH]);

	GetPropAsInt(GAME_CONFIG_VGRAVCYCLELIMIT, VehicleLimits[VEHICLE_GravCycle]);
	GetPropAsInt(GAME_CONFIG_VBEOWULFLIMIT, VehicleLimits[VEHICLE_Beowulf]);
	GetPropAsInt(GAME_CONFIG_VSHRIKELIMIT, VehicleLimits[VEHICLE_Shrike]);
	GetPropAsInt(GAME_CONFIG_VGRAVPRICE, VehiclePrices[VEHICLE_GravCycle]);
	GetPropAsInt(GAME_CONFIG_VSHRIKEPRICE, VehiclePrices[VEHICLE_Shrike]);
	GetPropAsInt(GAME_CONFIG_VBEOWULFPRICE, VehiclePrices[VEHICLE_Beowulf]);
    
    GetPropAsInt(GAME_CONFIG_CALLINSDENIED, CallinsDenied);
    GetPropAsInt(GAME_CONFIG_CALLINPRICETAC, CallinPrices[0]);
    GetPropAsInt(GAME_CONFIG_CALLINPRICESUP, CallinPrices[1]);
    GetPropAsInt(GAME_CONFIG_CALLINPRICEORB, CallinPrices[2]);
    GetPropAsInt(GAME_CONFIG_CALLINCLDWNTAC, CallinCooldowns[0]);
    GetPropAsInt(GAME_CONFIG_CALLINCLDWNSUP, CallinCooldowns[1]);
    GetPropAsInt(GAME_CONFIG_CALLINCLDWNORB, CallinCooldowns[2]);
    GetPropAsFloat(GAME_CONFIG_CALLINBLDUPTAC, CallinBuildups[0]);
    GetPropAsFloat(GAME_CONFIG_CALLINBLDUPSUP, CallinBuildups[1]);
    GetPropAsFloat(GAME_CONFIG_CALLINBLDUPORB, CallinBuildups[2]);        

	if (GetPropAsInt(GAME_CONFIG_FRIENDLYFIRE, IntVal)) bFriendlyFire = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_UPGRADES, IntVal)) bObjectiveUpgrades = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_AUTOBALANCE, IntVal)) bAutoBalanceInGame = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_GENAUTOREGEN, IntVal)) bGeneratorAutoRegen = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_POWERDEPLOYS, IntVal)) bPoweredDeployables = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_BASEASSETS, IntVal)) bPreplacedObjectives = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_GENDESTROYABLE, IntVal)) bGenDestroyable = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_FFBASEASSETS, IntVal)) bFriendlyFireBaseAssets = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_FFDEPLOYABLES, IntVal)) bFriendlyFireDeployables = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_SUPPLYDROPAMMOONLY, IntVal)) bSupplyDropAmmoOnly = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_POWEREDCALLINS, IntVal)) bPoweredCallins = IntVal > 0 ? true : false;
	if (GetPropAsInt(GAME_CONFIG_TEAMCREDITS, IntVal)) bTeamCredits = IntVal > 0 ? true : false;
    if (GetPropAsInt(GAME_CONFIG_SKIENABLED, IntVal)) bSkiEnabled = IntVal > 0 ? true : false;
    if (GetPropAsInt(GAME_CONFIG_CTFBLITZALLFLAGSMOVE, IntVal)) bCTFBlitzAllFlagsMove = IntVal > 0 ? true : false;
    
	GetPropAsFloat(GAME_CONFIG_FFMULTIPLIER, fFriendlyFireDamageMultiplier);
    fFriendlyFireDamageMultiplier /= 1000;

    GetPropAsFloat(GAME_CONFIG_VEHICLEHEALTHMULT, fVehicleHealthMultiplier);
    fVehicleHealthMultiplier /= 1000;

    GetPropAsFloat(GAME_CONFIG_ENERGYMULTIPLIER, fEnergyMultiplier);
    fEnergyMultiplier /= 1000;

    GetPropAsFloat(GAME_CONFIG_AOESIZEMULTIPLIER, fAoeSizeMultiplier);
    fAoESizeMultiplier /= 1000;

    GetPropAsFloat(GAME_CONFIG_AOEDAMAGEMULTIPLIER, fAoEDamageMultiplier);
    fAoEDamageMultiplier /= 1000;

	ApplyServerSettings();
}

simulated function int GetVehicleLimit(int Index)
{
    return VehicleLimits[Index];
}

simulated function bool IsCallinAllowed(int Index)
{
    return ((CallinsDenied >> Index) & 0x1) > 0 ? false : true;
}

simulated function bool IsClassAllowed(TrClassType ClassType, int CurrentCount)
{
    return (ClassCounts[ClassType] - CurrentCount > 0);
}

simulated function int GetClassCount(TrClassType ClassType)
{
    return ClassCounts[ClassType];
}

replication
{
	if (bNetInitial && Role == ROLE_Authority)
        TimeLimit, WarmupTime, ScoreLimit, MaxPlayers, RespawnTime, TeamAssignType, OvertimeLimit,
        ClassCounts, bGeneratorAutoRegen, BaseDestructionLimit, SpawnType, bFriendlyFire, bAutoBalanceInGame,
        bObjectiveUpgrades, bPoweredDeployables, bPreplacedObjectives, fFriendlyFireDamageMultiplier,
        CallinsDenied, CallinPrices, CallinBuildups, CallinCooldowns, VehicleLimits, VehiclePrices;		
}
...
defaultproperties
{
   ...
   RespawnTime=5
   OvertimeLimit=-1
   bAutoBalanceInGame=True
   bObjectiveUpgrades=True
   bPoweredDeployables=True
   bGeneratorAutoRegen=True
   bPreplacedObjectives=True
   bGenDestroyable=True
   bSkiEnabled=True
   GameScores(0)=5
   GameScores(1)=30
   GameScores(2)=100
   ...
   VehicleLimits(2)=2
   VehiclePrices(0)=500
   VehiclePrices(1)=4000
   VehiclePrices(2)=2500
   VehiclePrices(3)=10000
   VehiclePrices(4)=500
   CallinPrices(0)=4000
   CallinPrices(1)=2000
   CallinPrices(2)=10000
   CallinCooldowns(0)=30
   CallinCooldowns(1)=30
   CallinCooldowns(2)=30
   CallinBuildups(0)=3.000000
   CallinBuildups(1)=2.000000
   CallinBuildups(2)=4.000000
   fFriendlyFireDamageMultiplier=500.000000
   fVehicleHealthMultiplier=1000.000000
   fEnergyMultiplier=1000.000000
   fAoESizeMultiplier=1000.000000
   fAoEDamageMultiplier=1000.000000
   MaxSpeedWithFlagMedium=275
   DecelerationRateWithFlag=10
   MaxSpeedWithFlagLight=300
   MaxSpeedWithFlagHeavy=250
   ...
   Name="Default__TrServerSettingsInfo"
