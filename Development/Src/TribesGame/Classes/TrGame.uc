 ...
 * 
 */
class TrGame extends UTTeamGame
    config(Game)
    native;
...
var bool        m_bPostedStats;
var string      m_sWinnerName;
var int         m_nWinningTeam;
var int         m_nEndMatchCounter;
var int         m_nNextEffectInstanceId;

/** Overtime time limit. */
var float m_OvertimeTimeLimit;

var int MINIMUM_CREDITS_FOR_EXPERIENCE;
...
var bool bForceEndGame;
var bool bForceRoundStart;
var bool bFirstBloodAchieved;

/** Stats Interface. */
var TrStatsInterface Stats;

/** Audio Cue Variables */
var bool bPlayTeamAudio;
var bool bPlayedBEWinning;
var bool bPlayedDSWinning;

var bool PlayedEndOfGame;
var int  EndGameWait;
var int  SummaryWait;
var int  CountdownWait;

var int m_nPlayerTrackingCount;
var int m_nPlayerTrackingInterval;

var float m_fAutoBalanceTime;
var int m_nAutoBalanceTeamDifference;

/** Auto-balance enabled */
var bool m_bShouldAutoBalance;

/** Friendly fire */
var float m_fFriendlyFireDamageScale;

/** Is the player roaming a map? (Visiting it, no server, no XP, etc) */
var bool m_bRoamingMap;

/** Keeps track of tracking missiles and targeting saber launchers. SERVER ONLY! */
var TrSeekingMissileManager m_SeekingMissileManager;

/** How long before a forced respawn happens for a dead player. 0 means no forced respawn. */
var float m_fForcedRespawnTime;

/** How long our round countdown is. */
var int m_nRoundCountdownTime;

/** Our current countdown remaining time. */
var transient int m_nRoundCountdownRemainingTime;

/** Minumum amount of players that must be in the match for it to start. */
var int m_nMinNetPlayers;

native function int GetNextEffectInstanceId();

native function LocalLeaveGame();

native function OnServerInitialized();

native function SetOnlinePlayerName(PlayerReplicationInfo PRI);
...
simulated function ApplyServerSettings()
{
    ...
    if (TrGRI.r_ServerConfig.TimeLimit >= 0)
    {
        TimeLimit = TrGRI.r_ServerConfig.TimeLimit;
    }

    // DO NOT have the instance limit players.
    // GSC / match manager deal with this.
    //if (TrGRI.r_ServerConfig.MaxPlayers >= 0)
    //{
    //    MaxPlayers = TrGRI.r_ServerConfig.MaxPlayers;
    //    MaxPlayersAllowed = TrGRI.r_ServerConfig.MaxPlayers;
    //}

    if (TrGRI.r_ServerConfig.OvertimeLimit >= 0)
    {
        m_OvertimeTimeLimit = TrGRI.r_ServerConfig.OvertimeLimit;
    }

    if (TrGRI.r_ServerConfig.WarmupTime >= 0)
    {
        WarmupTime = TrGRI.r_ServerConfig.WarmupTime;
        m_nRoundCountdownTime = TrGRI.r_ServerConfig.WarmupTime;
    }

	m_bShouldAutoBalance = TrGRI.r_ServerConfig.bAutoBalanceInGame;
	m_fFriendlyFireDamageScale = TrGRI.r_ServerConfig.fFriendlyFireDamageMultiplier;
}

// Reduce damage if friendly fire is enabled.
function ReduceDamage(out int Damage, Pawn injured, Controller instigatedBy, vector HitLocation, out vector Momentum, class<DamageType> DamageType, Actor DamageCauser)
{
	local Controller InjuredController;

	// Apply friendly fire damage scaling.
	if (WorldInfo.GRI.r_bFriendlyFire && injured != none)
	{
		InjuredController = injured.Controller;
		if (InjuredController != instigatedBy && InjuredController != none && InjuredController.GetTeamNum() == instigatedBy.GetTeamNum())
		{
			Damage = float(Damage) * m_fFriendlyFireDamageScale;
			Momentum *= m_fFriendlyFireDamageScale;
		}
	}
...
function PreBeginPlay()
{
	...
	if (!m_bRoamingMap && Stats == none)
	{
	    Stats = new class'TrStatsInterface';
	    Stats.Initialize(OnlineSub);
	}

	if( m_SeekingMissileManager == none )
	{
		m_SeekingMissileManager = Spawn(class'TrSeekingMissileManager',,,,,,true);
	}

	// Apply the server settings.
	ApplyServerSettings();

	if (m_bShouldAutoBalance)
	{
		SetTimer(5.0f, true, 'CheckForAutoBalance');
	}
}
...
event KickIdler(PlayerController PC)
{
	...
	{
		;
		TrPC.ServerKickPlayer();
	}
}

event PlayerController Login(string Portal, string Options, const UniqueNetID UniqueID, out string ErrorMessage)
{
    local TrPlayerController NewPlayer;
    
    NewPlayer = TrPlayerController(super.Login(Portal, Options, UniqueID, ErrorMessage));
    
    // JCR - Go to MCTS and request our name there, the Options URL will fail on parsing some name symbols
    SetOnlinePlayerName(NewPlayer.PlayerReplicationInfo);

    if (Stats != none) Stats.SetIdentification(NewPlayer);

    return NewPlayer;
}

// Overriden to allow for players to spawn on top of each other.
...
function int DetermineWinningTeam()
{
    ...
    {
        WinningTeam = Teams[0].Score > Teams[1].Score ? 2 : 1;
    }
	return WinningTeam;
}
...
function EndGame( PlayerReplicationInfo Winner, string Reason )
{
	...
	if ( !CheckEndGame(Winner, Reason) && !bForceEndGame)
	{
		TrGRI = TrGameReplicationInfo(GameReplicationInfo);
		
		if( !bOverTime && TrGRI != none )
		{
			// Set a new overtime time limit.
			TrGRI.TimeLimit = m_OvertimeTimeLimit;
			TrGRI.RemainingTime = (m_OvertimeTimeLimit * 60.0) + 1;
			TrGRI.RemainingMinute = m_OvertimeTimeLimit * 60.0; // This gets replicated to clients.
			BroadcastLocalizedMessage(class'TrGameMessage', 3);
			bOverTime = true;
		}
	...
	if (Role == ROLE_Authority && !m_bPostedStats)
    {
        m_bPostedStats = true;
		m_nWinningTeam = DetermineWinningTeam();
        ...
        if (Stats != none) Stats.GameEnd();

		foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
		{
			// Set time played on class and weapons
			TrPC.AddTimePlayed();
			TrPC.MatchEndSoundMode();

            TrPC.m_AccoladeManager.GameCompleted();

            // Game winning accolade code, pardon the absurd comparisons
            if (TrPC.PlayerReplicationInfo.PlayerName == m_sWinnerName || 
               ((TrPC.GetTeamNum() == 0 && m_nWinningTeam == 2) ||
                (TrPC.GetTeamNum() == 1 && m_nWinningTeam == 1)))
            {
                TrPC.m_AccoladeManager.GameWon();
            }
		}
        
	    foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
	    {
	        TrPC.SummarizeAwards();
        }

        PRIArray = WorldInfo.Game.GameReplicationInfo.PRIArray;

        if (WorldInfo.GRI.GameClass != class'TrGame_TRRabbit')
        {
            PRIArray.Sort(CreditSort);
        }
        else
        {
            PRIArray.Sort(ScoreSort);
        }

		if (Stats != none)
		{
			Stats.WriteMatchStats();
			Stats.WritePlayerStats(PRIArray);
			Stats.Flush();
		}
	}
...
state MatchOver
{
	...
	function Timer()
	{
        ...
        if (m_nEndMatchCounter >= CountDownWait)
        {
            SendNextMatchCountdown(EndGameWait - m_nEndMatchCounter);
        }

        if (m_nEndMatchCounter == SummaryWait)
        {
            SendShowSummary();
        }
...
state MatchInProgress
{
	function Timer()
	{
		...
		else
		{
			...
			{
				// Overtime has run out. Match is a draw.		
				EndGame(None, "MatchDraw");
			}
			else
			{
				super.Timer();
			...
			m_nPlayerTrackingCount++;

            // JCR - Membase is not set up right now, no need for the server to chug on this
			//if (m_nPlayerTrackingCount >= m_nPlayerTrackingInterval) TrackPlayerMovement();

			if (TimeLimit > 0)
			{
				if (bPlayTeamAudio && GameReplicationInfo.RemainingTime <= (15 * TimeLimit))
				{
...
function TrackPlayerMovement()
{
	...
	if (Role != ROLE_Authority || Stats == none) return;

    for (i = 0; i < GameReplicationInfo.PRIArray.Length; i++)
    {
        Stats.TrackPlayerMovement(GameReplicationInfo.PRIArray[i], WorldInfo.TimeSeconds);
    }
    
    m_nPlayerTrackingCount = 0;*/
}
...
function RespawnPlayers()
{
	...
	foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
	{
		if( TrPC.GetTeamNum() != 255 || (WorldInfo.NetMode == NM_Standalone || WorldInfo.NetMode == NM_ListenServer) )
		{
...
ignores CheckScore, ScoreKill;
    ...
    function ForceRoundStart()
    {
        bForceRoundStart = true;
        m_nRoundCountdownRemainingTime = 0;
    }
	...
	function Timer()
	{
		...
		if( TrGRI != none )
		{
			if( bForceRoundStart || (NumPlayers >= m_nMinNetPlayers && Teams[0].Size > 0 && Teams[1].Size > 0) )
			{
...
State PendingMatch
{
    ...
    function Timer()
	{
        ...
        else
        {
			...
			if (bWarmupRound)
			{
				...
				{
                    // Match begins now.
                    Broadcast(self, "Round Started!"); //TODO: Localize this.
					WarmupRemaining = -1;
					TrGameReplicationInfo(GameReplicationInfo).SetWarmingUp(false);
					bWarmupRound = false;
			   		ResetLevel();
                    StartMatch();
                    PlayStartupMessage();
    ...
    function BeginState(Name PreviousStateName)
    {
		...
		TrGameReplicationInfo(GameReplicationInfo).CanPlaceDeployables(true);

		foreach WorldInfo.AllControllers(class'PlayerController', PC)
		{
			TrPlayerController(PC).ResetSoundMode();
		}

        // Don't go through PendingMatch warmpup stage if standalone.
        ...
        if (bWarmupRound)
		{
            // Change the spawn timer between shipping PC and local. This is so artists don't have to wait so long to spawn.
...
//
//	override constants and add credit assignment
//
function NotifySpree(UTPlayerReplicationInfo Other, int num)
{
	local TrPlayerController TrPC;

	TrPC = TrPlayerController(Other.Owner);

	if (TrPC != none)
	{
		TrPC.m_AccoladeManager.UpdateStandardKillingSpree(Other, num);
	}
}
...
function EndSpree(UTPlayerReplicationInfo Killer, UTPlayerReplicationInfo Other)
{
	...
	{
		TrPlayerController(Killer.Owner).m_AccoladeManager.NoJoy();
	...
	foreach WorldInfo.AllControllers(class'PlayerController', PC)
	{
		...
		{
			PC.ReceiveLocalizedMessage( class'TrKillingSpreeMessage', 1, None, Other );
		}
		else
		{
			PC.ReceiveLocalizedMessage( class'TrKillingSpreeMessage', 0, Other, Killer );
		}
	}
}
...
function GenericPlayerInitialization(Controller C)
{
	...
	if ( !bUseClassicHUD )
	{
		HUDType = class'TrHud'; //bTeamGame ? class'TrHUDTeam' : class'TrHUD';
	}

	TRPC = TrPlayerController(C);
	if(TRPC!=None)
	{
        // JCR - Why are you taking my credits away!?
		//TRPC.ModifyCredits(-TRPC.GetCurrentCredits());
	}

	// seamless travel destroys team object for non-team games; Rabbit needs team
	// for re-matches in rabbit, need to set team
	if (Role==ROLE_Authority && WorldInfo.GRI.GameClass == class'TrGame_TRRabbit' && TRPC.PlayerReplicationInfo.Team==none)
	{
		TRPC.SwitchTeam();
	}
...
static event class<GameInfo> SetGameType(string MapName, string Options, string Portal)
{
    ...
    // Default un prefixed maps to Team Death Match.
    return class'TrGame_TRTDM';
}

function PlayEndOfMatchMessage()
{
    if (PlayedEndOfGame) return;

	if( Teams[0].Score == Teams[1].Score )
	{
		BroadcastLocalizedMessage(class'TrVictoryMessage', 7);
	}
	else
	{
		BroadcastLocalizedMessage(class'TrVictoryMessage', Teams[0].Score > Teams[1].Score ? 0 : 1);
	}
    
    PlayedEndOfGame = true;
}
...
function SendMatchOver()
{
	...
	foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
	{
        TrPC.ClientMatchOver(m_nWinningTeam, m_sWinnerName);
	}
}

function SendShowSummary()
{
	local TrPlayerController TrPC;

	foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
	{
        TrPC.ClientSetEndOfMatch();
        TrPC.ClientFadeToSummary();
	}
}

function SendNextMatchCountdown(int Seconds)
{
	local TrPlayerController TrPC;

	foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
	{
        TrPC.UpdateMatchCountdown(Seconds);
	}
}
function SendMatchCountdown(int Seconds)
{
	...
	foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
	{
		TrPC.ReceiveLocalizedMessage(class'TrGameStartMessage', Seconds);
	}
}

// Override so we can process credit events
function ScoreKill(Controller Killer, Controller Other )
{
	local bool bIsFriendlyFire;
    local int i;
	local TrPlayerController TrKiller, FlagHolderPC, TrVictim, TrAssister, TrWeapAssister;
	local TrGame_TRRabbit RabbitGame;
	local TrGame_TRTeamRabbit TeamRabbitGame;
	local TrPawn TrP;
	local bool bAssistedSuicide; // Killer is assigned if their last damaged pawn suicides.
	local TrDeployableController DeployableKillerController;
	local TrDeployable DeployableKiller;
	local TrTurretPawn TurretPawn;
	local TrProjectile KilledByProjectile;
	local TrVehicle VehicleKiller;

    if (Killer == none || Killer == Other)
	{
		// Suicide, find the last damager.
		TrP = TrPawn(Other.Pawn);
		if( TrP != none )
		{
			Killer = TrP.GetLastDamager();

			if( Killer == none )
			{
				// There wasn't a last damager.
				if( TrPlayerController(Killer) != none )
				{
					TrPlayerController(Killer).ClearDetermination();
				}
				return;
			}
			else
			{
				BroadcastLocalized(self,class'TrKillerMessage', 2, Killer.PlayerReplicationInfo, Other.PlayerReplicationInfo);
				bAssistedSuicide = true;                
			}
		}
		else
		{
			// Somehow there was no pawn.
			return;
		}
	}
    else if (Killer == none)  //TODO: remove this after patch (paranoia!)
    {
        return;
    }
    
    //TODO: remove the Killer != none check after patch (paranoia!)
    bIsFriendlyFire = Killer != none && Other != none && Killer.GetTeamNum() == Other.GetTeamNum();

	DeployableKillerController = TrDeployableController(Killer);
	if( DeployableKillerController != none )
	{
		TurretPawn = TrTurretPawn(DeployableKillerController.Pawn);
		if( TurretPawn != none )
		{
			DeployableKiller = TurretPawn.m_OwnerDeployable;
		}

		TrKiller = DeployableKillerController.m_SpawnedFromController;
		Killer = TrKiller;

        if (Stats != none) Stats.AddDeployableDestruction(TrKiller);
	}
	else
	{
	...
	// Only let the server update accolades and credits.
	if (Role == ROLE_Authority )
	{
		TrVictim = TrPlayerController(Other);
		if( TrVictim != none )
		{
			if( DeployableKiller != none )
			{
				// Killed by a deployable, such as a turret. Favor this.
				TrVictim.r_KilledBy = DeployableKiller;
			}
			else if( TrKiller != none && TrKiller.IsInVehicle() )
			{
				VehicleKiller = TrVehicle(TrKiller.Pawn);

                if (VehicleKiller == none)
                {
                    VehicleKiller = TrVehicle(TrKiller.Pawn.Base);
                }

				if( VehicleKiller != none )
				{
					TrVictim.r_KilledBy = VehicleKiller;

                    // See if we need to give out any assists (non friendly fire)
					if (!bIsFriendlyFire)
					{
						for (i = 0; i < VehicleKiller.Seats.Length; i++)
						{
							if (VehicleKiller.Seats[i].StoragePawn == none)
								continue;

							TrAssister = TrPlayerController(VehicleKiller.Seats[i].StoragePawn.Owner);
							TrWeapAssister = TrPlayerController(VehicleKiller.Seats[i].StoragePawn.Owner.Owner);

							if (TrAssister != none && TrAssister != TrKiller)
							{
								TrAssister.m_AccoladeManager.GiveVehicleAssist();
							}
							else if (TrWeapAssister != none && TrWeapAssister != TrKiller)
							{
								TrWeapAssister.m_AccoladeManager.GiveVehicleAssist();
							}
						}
					}
				}
			}
			else if( TrKiller != none && TrKiller.Pawn != none )
			{
				TrVictim.r_KilledBy = TrKiller.Pawn;

				KilledByProjectile = TrProjectile(TrVictim.m_LastDamagedBy);
				if( KilledByProjectile != none && KilledByProjectile.m_SpawnedEquipPoint == EQP_Belt && !bIsFriendlyFire )
				{
					// Kill was from an offhand.
					TrKiller.ProcessCreditEvent(OffhandKillCredit, false);
				}
			}
		}

		// Process appropriate credit event based on gametype
		// Must put this code here as these kills override the normal credits given for a kill
		if( WorldInfo.GRI.GameClass == class'TrGame_TRRabbit' )
		{
			RabbitGame = TrGame_TRRabbit(WorldInfo.Game);
			if( RabbitGame != none && RabbitGame.m_HolderPRI != none && !bIsFriendlyFire )
			{
				FlagHolderPC = TrPlayerController(RabbitGame.m_HolderPRI.Owner);
				if( FlagHolderPC != none && TrKiller != none )
				{
					// Determine if I killed the Rabbit or if I got a kill as the Rabbit
					if (TrKiller == FlagHolderPC)
					{
						TrKiller.m_AccoladeManager.KillAsRabbit(Killer, Other);
					}
					else
					{
						TrKiller.m_AccoladeManager.KilledTheRabbit(Killer, Other);
					}
				}
			}
		}
        else if( WorldInfo.GRI.GameClass == class'TrGame_TRTeamRabbit' )
		{
			TeamRabbitGame = TrGame_TRTeamRabbit(WorldInfo.Game);

			if (TeamRabbitGame != none && TeamRabbitGame.m_TeamRabbitFlag != none &&
			    TeamRabbitGame.m_TeamRabbitFlag.Holder != none && TeamRabbitGame.m_TeamRabbitFlag.Holder.Owner != none && !bIsFriendlyFire)
			{
				FlagHolderPC = TrPlayerController(TeamRabbitGame.m_TeamRabbitFlag.Holder.Owner);

				if (FlagHolderPC != none && TrKiller != none)
				{
					if (TrVictim == FlagHolderPC)
					{
						TrKiller.m_AccoladeManager.KilledTheFlagHolder();
					}
					else
					{
			            TrKiller.ProcessCreditEvent(PlayerKillEvent, false);
					}
				}
			}
		}
		...
		{
			if (TrKiller != none && !bIsFriendlyFire)
			{
				TrKiller.ProcessCreditEvent(PlayerKillEvent, false);
			}
		}
		
        if (Stats != none && TrKiller != none)
        {
		    Stats.AddKill(TrKiller);
        }

		if (TrKiller != none)
		{
			if( !bAssistedSuicide && !bIsFriendlyFire )
			{
				// Give accolades.
				TrKiller.m_AccoladeManager.UpdateSpecialAccolades(Other);
			}

			// Don't accumulate any determination since TrKiller has killed someone.
			TrKiller.ClearDetermination();
		}
	}

	if (Role == ROLE_Authority && Other.Pawn != none && Other.Pawn.IsA('TrPawn'))
	{
		// Process Assists
		TrPawn(Other.Pawn).ProcessKillAssists(Killer);
	}
	
    // Update our kills for the leaderboard and stats
	if (TrKiller != none && TrKiller.PlayerReplicationInfo.IsA('TrPlayerReplicationInfo'))
	{
		// Deduct kills for friendly fire.
		if (bIsFriendlyFire)
		{
			TrKiller.ProcessCreditEvent(TeamKillCredit, false);
			TrPlayerReplicationInfo(TrKiller.PlayerReplicationInfo).m_nKills--;
		}
		else
		{
			TrPlayerReplicationInfo(TrKiller.PlayerReplicationInfo).m_nKills++;
    ...
    // Bypass the UTTeamGame scoring algorithm (it gives bonus points for game-specific scenerios).
    super(UTGame).ScoreKill(Killer,Other);
}
...
function RestartPlayer(Controller aPlayer)
{
	...
	// Make sure respawning players do not have the flag.
	TrPRI = TrPlayerReplicationInfo(aPlayer.PlayerReplicationInfo);
	if (TrPRI != none)
	{
		TrPRI.SetFlag(none);
	}
...
function bool ChangeTeam(Controller Other, int num, bool bNewTeam)
{
    ...
    NewTeam = (num < 255) ? Teams[num] : None;
...
/** 
 *  Called from GameInfo::Login() -- this should not decide teams for a team-based game. 
 *  Team selection is now controlled by a request system by the player.
 */
...
{
	// If we were assigned to a team previously, just switch teams.
	if (C.PlayerReplicationInfo != none && C.PlayerReplicationInfo.Team != none)
	{
		return (C.PlayerReplicationInfo.Team.TeamIndex + 1) % 2;
	}
 ...
 *  Returns TRUE if request is granted, otherwise returns false.
 *  */
function bool RequestTeam(byte RequestedTeamNum, Controller C)
{
	...
	local TrDeployable_Sensor Sensor;
	local TrPlayerController TrPC;
	local TrGameReplicationInfo TrGRI;

    TrGRI = TrGameReplicationInfo(GameReplicationInfo);
    
    if (TrGRI != none && !TrGRI.CanJoinTeam(RequestedTeamNum, C.GetTeamNum()))
    {
        return false;
    }
    ...
    if (TrPC != none)
	{
		TrPC.m_bJustJoined = false;
	}

	if( RequestedTeamNum == 255 )
	{
		// Requesting to be a spectator.
		SetTeam(C, none, true);
	}
	else
	{
		SetTeam(C, Teams[RequestedTeamNum], true);
	}
	...
	if (OnlineSystem != none)
	{
		...
		{
			// Notify the chat system of the team change.
			ChatInterface.NotifyTeamChange(C.PlayerReplicationInfo.UniqueId.Uid, RequestedTeamNum);
		}
	}

	// If a team swap occured, update radar sensors.
	foreach DynamicActors(class'TrDeployable_Sensor', Sensor)
	{
		// Refresh and force a scan.
		Sensor.ReleaseAllDetectedPawns();
		Sensor.ScanTargets();
	}
...
function bool AutoAssignTeam(Controller C)
{
	if (C != none && C.GetTeamNum() != 255)
	{
		return false;
	}

    if ((Teams[0].Size - Teams[1].Size) > 0)
    {
	    SetTeam(C, Teams[1], true);
    }
    else
    {
	    SetTeam(C, Teams[0], true);
    }
...
event HandleSeamlessTravelPlayer(out Controller C)
{
	...
	{
		NewTeamIndex = PickTeamForMigration(C);

		if (C.PlayerReplicationInfo.Team != None && NewTeamIndex < ArrayCount(Teams))
		{
			if (Teams[NewTeamIndex] != C.PlayerReplicationInfo.Team)
			{
		...
		{
			ChangeTeam(C, NewTeamIndex, false);
		}
	}
    ...
    TrPawn(TrPlayerController(PC).Pawn).ActivatePendingClass();
}
...
function SetPlayerDefaults(Pawn PlayerPawn)
{
	...
	local TrPawn ATrPawn;

	ATrPawn = TrPawn(PlayerPawn);
	if (ATrPawn == none)
	{
	...
	FamilyInfo = class<TrFamilyInfo>(ATrPawn.GetFamilyInfo());
	if (FamilyInfo == none)
	{
		super.SetPlayerDefaults(PlayerPawn);
		return;
...
{
	local TrPlayerController KillerTrPC, TrKilledPlayer;

	TrKilledPlayer = TrPlayerController(KilledPlayer);
    
    if (Stats != none) Stats.AddDeath(TrKilledPlayer);
    
	// Figure out who the actual killing player was
    if (TrDeployableController(Killer) != none) 
    {
    	KillerTrPC = TrDeployableController(Killer).m_SpawnedFromController;
    }
	else 
	{
		KillerTrPC = TrPlayerController(Killer);
	}

    if (KillerTrPC != none) Killer = KillerTrPC;

    // Update heat map tracking as well for a kill
    // JCR - We aren't using this data right now, don't make the server chug on it
    /*if (KillerTrPC != none && KilledPawn != none && (KillerTrPC != TrKilledPlayer))
    {
        AddHeatMapRecord(TrPlayerReplicationInfo(KillerTrPC.PlayerReplicationInfo), TrPlayerReplicationInfo(KilledPawn.PlayerReplicationInfo), Killer.Pawn.Location, KilledPawn.Location, damageType, KilledPawn.Weapon, TrPawn(KilledPawn).m_fLastDamagerTimeStamp);
    }*/

    if (TrKilledPlayer != none) 
    {
		if( TrKilledPlayer == KillerTrPC )
		{
            if (class<TrDmgType_Fell>(damageType) != none && TrKilledPlayer.Stats != none)
            {
                TrKilledPlayer.Stats.FallingDeath(TrKilledPlayer);
            }
			// Removed the credit penalty for now.
			/*
			TrKilledPlayer.ProcessCreditEvent(SuicideEvent, false);
			*/
		}
		else
		{
			// Only opposing team members get credit for dying.
			if (KilledPlayer.GetTeamNum() != Killer.GetTeamNum())
			{
    			TrKilledPlayer.ProcessCreditEvent(PlayerDeathEvent, false);
			}

            if ((class<TrDmgType_DeliveryPod>(damageType) != none ||
                 class<TrDmgType_OrbitalStrike>(damageType) != none ||
                 class<TrDmgType_TacticalStrike>(damageType) != none) &&
                 KillerTrPC.Stats != none)
            {
                KillerTrPC.Stats.CallInKill(KillerTrPC);
            }

            if (class<TrDmgType_Base>(damageType).default.bOffhandType && KillerTrPC.Stats != none)
            {
                KillerTrPC.Stats.BeltKill(KillerTrPC);
            }
		}
    }
	...
	if( KillerTrPC != none )
	{
		KillerTrPC.UpdateNewPlayerAssistPoints(2);
	}
}

// Method of determining player rank based on credits earned or score during the match
delegate int ScoreSort(PlayerReplicationInfo A, PlayerReplicationInfo B) { return A.Score < B.Score ? -1 : 0; }
delegate int CreditSort(PlayerReplicationInfo A, PlayerReplicationInfo B) { return TrPlayerReplicationInfo(A).m_nCreditsEarned < TrPlayerReplicationInfo(B).m_nCreditsEarned ? -1 : 0; }

delegate int CreditsSortViaPC(PlayerController A, PlayerController B)
{
	local TrPlayerReplicationInfo A_PRI, B_PRI;

	A_PRI = TrPlayerReplicationInfo(A.PlayerReplicationInfo);
	B_PRI = TrPlayerReplicationInfo(B.PlayerReplicationInfo);

	// Do a credit sort... unless A has the flag. Then ignore them.
	if (A_PRI != none && B_PRI != none && 
		!A_PRI.bHasFlag && 
		A_PRI.m_nCreditsEarned < B_PRI.m_nCreditsEarned)
		return -1;
...
// Auto-Balancing
function bool AreTeamsUnbalanced()
{
	local byte currTeamNum;
	local int relativeTeamSize;
	local PlayerController CurrPC;

	// Loop through all player controllers, because apparently checking team size isn't trustworthy.
	foreach WorldInfo.AllControllers(class'PlayerController', CurrPC)
	{
		currTeamNum = CurrPC.GetTeamNum();

		// Determine the relative team sizes.
		if (currTeamNum == 255)
			continue;
		else if (currTeamNum == 0)
			relativeTeamSize++;
		else
			relativeTeamSize--;
	}

	if (Abs(relativeTeamSize) >= m_nAutoBalanceTeamDifference)
	{
		return true;
	}
	else
...
{
	// Auto-balance isn't enabled.
	if (!m_bShouldAutoBalance)
		return;

	// We are already going to auto-balance teams, so just end.
	if (IsTimerActive('AutoBalanceTeams'))
		return;

	// Is there a team size discrepancy?
	if (AreTeamsUnbalanced())
	{
		SetTimer(m_fAutoBalanceTime - 20.0f, false, 'AutoBalanceInTwenty');
		SetTimer(m_fAutoBalanceTime - 5.0f, false, 'AutoBalanceInFive');
		SetTimer(m_fAutoBalanceTime, false, 'AutoBalanceTeams');
		return;
	}
}

function AutoBalanceInTwenty()
{
	// Make sure teams are still unbalanced.
	if (AreTeamsUnbalanced())
	{
		BroadcastLocalizedMessage(class'TrAutoBalanceMessage', 0);
	}
	else
	{
		ClearTimer('AutoBalanceInFive');
		ClearTimer('AutoBalanceTeams');
	}
}

function AutoBalanceInFive()
{
	// Make sure teams are still unbalanced.
	if (AreTeamsUnbalanced())
	{
		BroadcastLocalizedMessage(class'TrAutoBalanceMessage', 1);
	}
	else
	{
		ClearTimer('AutoBalanceTeams');
...
{
	local array<PlayerController> BEArray;
	local array<PlayerController> DSArray;
	local byte currTeamNum;
	local int relativeTeamSize;
	local PlayerController CurrPC;
	...
	foreach WorldInfo.AllControllers(class'PlayerController', CurrPC)
	{
		...
		{
			relativeTeamSize++;
			BEArray.AddItem(CurrPC);
		}
		else
		{
			relativeTeamSize--;
			DSArray.AddItem(CurrPC);
		}
	}
	...
	// Switch a random person from the team with the most people.
	if (relativeTeamSize < 0)
		MoveRandomPlayerFromTeam(DSArray, Abs(relativeTeamSize) / 2);
	else
		MoveRandomPlayerFromTeam(BEArray, Abs(relativeTeamSize) / 2);
}

function MoveRandomPlayerFromTeam(array<PlayerController> TeamList, int PlayersToMove)
{
	local int ArrayOffset;
	local TrPlayerController PCToMove;
	local TrPlayerReplicationInfo TrPRI;
	local TrVehicle TrV;

	// Sort by credits.
	TeamList.Sort(CreditsSortViaPC);

	// Grab some mediocre person.
	ArrayOffset = TeamList.Length / 2;

	while (PlayersToMove > 0)
	{
		// We may have some additional players to balance, and the player we want to balance has the flag.
		// So just abort and go through auto-balance again if necessary.
		if (ArrayOffset < 0)
			return;

		// Determine who to move and decrement our variables.
		PCToMove = TrPlayerController(TeamList[ArrayOffset]);
		TrPRI = TrPlayerReplicationInfo(PCToMove.PlayerReplicationInfo);
		TrV = TrVehicle(PCToMove.Pawn);
		ArrayOffset--;

		// If the player we want to move has the flag, go to the next one.
		if (TrPRI != none && TrPRI.bHasFlag)
			continue;

		// If the player we want to move is in a vehicle, go to the next one.
		if (TrV != none)
			continue;
		...
		if (PCToMove != none)
		{
            PCToMove.bWasAutoBalanced = true;
			PCToMove.ServerRequestPickTeam((PCToMove.GetTeamNum() + 1) % 2);
		}
	}
}
...
event InitGame( string Options, out string ErrorMessage )
{
	...
	InOpt = ParseOption(Options, "m_bRoamingMap");
	if (InOpt != "")
	{
		m_bRoamingMap = true;
	}
...
function SetPRI(PlayerController PC, PlayerReplicationInfo NewPRI)
{
    local int prevFree;
    local int prevEarned;
    local int prevCurrent;
    local TrPlayerController TrPC;
    local TrPlayerReplicationInfo TrPRI;

    TrPC  = TrPlayerController(PC);
    TrPRI = TrPlayerReplicationInfo(NewPRI);

    PC.PlayerReplicationInfo = TrPRI;

    // Credit fun - Save off our old values and re-add them as if we are gaining
    // them right now. This allows all the right stuff to happen for us.
    if (TrPC != none)
    {
        prevFree = TrPRI.m_nFreeCredits;
        prevEarned = TrPRI.m_nCreditsEarned;
        prevCurrent = TrPRI.m_nCurrentCredits;

        TrPRI.m_nFreeCredits = 0;
        TrPRI.m_nCreditsEarned = 0;
        TrPRI.m_nCurrentCredits = 0;

        TrPC.ModifyCredits(prevEarned - prevFree, false);
        TrPC.ModifyCredits(prevFree, true);
        TrPC.r_nCurrentCredits = prevCurrent;
    }
}

function int GetRankFromXP(int XP)
{
    local int i;
    ...
    for (i = 1; i < 51; i++)
    {
        ...
        if (XP == rank.default.RequiredXP)
        {
            return rank.default.Rank;
        }
        else if (XP > rank.default.RequiredXP)
        {
            prevRank = rank;
        }
        else if (XP < rank.default.RequiredXP)
        {
            return prevRank.default.Rank;
        }
    }
...
function BroadcastAccolade(string Message)
{ 
    local TrPlayerController PC;

	foreach WorldInfo.AllControllers(class'TrPlayerController', PC)
	{
        PC.ClientShowAccoladeText(Message, 2.0f);
	}
}

function SendMessageToAll(class<LocalMessage> InMessageClass, int Switch)
{
	local TrPlayerController PC;

	foreach WorldInfo.AllControllers(class'TrPlayerController', PC)
	{
        PC.ReceiveLocalizedMessage(InMessageClass, Switch);
	}
}
...
event int FindGameTypeId()
{
    return GetGameTypeId();
}
...
event TgChangeScore(int nTeam, optional int nCount)
{
    Teams[nTeam].Score += nCount;
}

event TgChangeTime(int nSeconds)
{
    WorldInfo.Game.GameReplicationInfo.RemainingTime = nSeconds;
}
...
function NavigationPoint FindPlayerStart(Controller Player, optional byte InTeam, optional string incomingName)
{
	...
	if( Best != none && Player != none )
	{
		Player.StartSpot = Best;
	}
...
defaultproperties
{
   m_bShouldAutoBalance=True
   m_sWinnerName="Player"
   m_OvertimeTimeLimit=10.000000
   MINIMUM_CREDITS_FOR_EXPERIENCE=200
   EndGameWait=44
   SummaryWait=6
   CountdownWait=6
   m_fAutoBalanceTime=120.000000
   m_nAutoBalanceTeamDifference=2
   m_nRoundCountdownTime=20
   m_nMinNetPlayers=2
   bWarmupRound=True
   WarmupTime=5
   ResetTimeDelay=30
   ConsolePlayerControllerClass=Class'TribesGame.TrConsolePlayerController'
   bTeamGame=False
   DefaultPawnClass=Class'TribesGame.TrPlayerPawn'
   HUDType=Class'TribesGame.TrHUD'
   DeathMessageClass=Class'TribesGame.TrDeathMessage'
   BroadcastHandlerClass=Class'TribesGame.TrBroadcastHandler'
   PlayerControllerClass=Class'TribesGame.TrPlayerController'
   PlayerReplicationInfoClass=Class'TribesGame.TrPlayerReplicationInfo'
   GameReplicationInfoClass=Class'TribesGame.TrGameReplicationInfo'
   Name="Default__TrGame"
