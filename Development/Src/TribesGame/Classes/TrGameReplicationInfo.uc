class TrGameReplicationInfo extends UTGameReplicationInfo
	native;
...
const ITEM_VOICE_T2BDERM03                              = 8726;#linenumber 5

/** This is our global Tribes object pool that will hold all of our pools of objects **/
var TrObjectPool m_TrObjectPool;
var init array<TrDeployable>            m_Deployables;

var PlayerReplicationInfo m_RabbitLeaderBoard[3];

/** PRI of the current rabbit. */
var PlayerReplicationInfo m_CurrentRabbitPRI;

/** The CTF flags */
var int FlagReturnTime[2];
var TrFlagBase m_Flags[2];

/** The server settings to use */
var TrServerSettingsInfo r_ServerConfig;

var int GeneratorPower[2];
var int GeneratorDowntime[2];

var bool r_bCanPlaceDeployables;

/** Resets the game. */
var byte r_ResetGame;

/** 'Flash' down when phase 0 on siege ends.*/
var byte r_nPhase0SeigeEnded;

/** Cache of all team blockers in a map. */
var array<TrTeamBlockerStaticMeshActor> m_TeamBlockerCache;

/** Current opacity for the last radar blip. */
var transient float m_fCurrentSensorBlipOpacity;

/** How fast to fade out from a blip. */
var float m_fSensorBlipFadeSpeed;

/** Increments every time a blip occurs. Online radar sensors can use this to show enemies. */
var repnotify byte r_nBlip;

/** */
var private float r_fGlobalSpeedCapMultiplier;

/** */
var private float r_fGlobalAccelerationMultiplier;

/** */
var private float r_fGlobalAirSpeedMultiplier;

var private float r_fGlobalMaxJetpackThrustSpeedMultiplier;

var(MaxAccel) PhysicsType r_PhysType;

/** Storm game type: Shield health. */
var int r_CarrierShieldHealth[2];

/** Storm game type: Core health. */
var int r_CarrierCoreHealth[2];

/** Keeps track of the score for the rounds. */
var int r_nRoundScore[2];

/** The current round number.*/
var int r_nCurrentRound;

/** Defend and Destroy game type: Cores. */
var TrDaDCore r_DaDCore[2];

/** Defend and Destroy game type: Pointers to BE Capacitors */
var TrDaDCapacitor r_DaDCapacitorBE[3];

/** Defend and Destroy game type: Pointers to DS Capacitors */
var TrDaDCapacitor r_DaDCapacitorDS[3];

/** Capture and Hold game type: Number of capture points held by each team. */
var int r_nNumCapturePointsHeld[2];

var bool r_bPoweredCallins;
var bool bGivePrivateXP;
var bool bGivePerfBonus;

var repnotify bool r_bSkiEnabled;

const MAX_NUM_ARENA_PLAYERS_PER_TEAM = 8;
const MIN_PLAYERS_FOR_PERF_BONUS = 6;
const MIN_PLAYERS_FOR_PRIVATE_XP = 8;

var TrPlayerReplicationInfo r_BEPlayerList[MAX_NUM_ARENA_PLAYERS_PER_TEAM];
var TrPlayerReplicationInfo r_DSPlayerList[MAX_NUM_ARENA_PLAYERS_PER_TEAM];

replication
{
	if (bNetDirty && Role == ROLE_Authority)
		FlagReturnTime, GeneratorPower, GeneratorDowntime, m_RabbitLeaderBoard, m_CurrentRabbitPRI, r_bCanPlaceDeployables, 
		m_Flags, r_nPhase0SeigeEnded, r_ResetGame, r_nBlip, r_fGlobalSpeedCapMultiplier, r_fGlobalAccelerationMultiplier, r_fGlobalAirSpeedMultiplier, r_fGlobalMaxJetpackThrustSpeedMultiplier, 
		r_ServerConfig, r_PhysType, r_CarrierShieldHealth, r_CarrierCoreHealth, r_nRoundScore, r_nCurrentRound, r_BEPlayerList, r_DSPlayerList, r_DaDCore, r_DaDCapacitorBE, r_DaDCapacitorDS,
		r_nNumCapturePointsHeld, r_bPoweredCallins, r_bSkiEnabled;
}
...
{
	r_ServerConfig = Spawn(class'TrServerSettingsInfo');
	if (r_ServerConfig != none)
	{
		r_ServerConfig.LoadServerSettings();
		ApplyServerSettings();
        bNetDirty = true;
	}
...
{
	local TrTeamBlockerStaticMeshActor TB;

	Super.PostBeginPlay();

	// create the object pool
	m_TrObjectPool = Spawn( class'TrObjectPool' );

	if( GameClass == class'TrGame_TrSiege' || WorldInfo.GetGameClass() == class'TrGame_TrSiege' )
	{
		ForEach AllActors( class 'TrTeamBlockerStaticMeshActor', TB )
		{
			if( TB.m_bDisableBlockingOnSiegePhase0Ends )
			{
				m_TeamBlockerCache.AddItem(TB);
			}
		}
	}
}
...
{
	// Should friendly fire be enabled?
	r_bFriendlyFire = r_ServerConfig.bFriendlyFire;
    r_bPoweredCallins = r_ServerConfig.bPoweredCallins;
    r_bSkiEnabled = r_ServerConfig.bSkiEnabled;
}

simulated function CanPlaceDeployables(bool value)
{
	r_bCanPlaceDeployables = value;
	bNetDirty = true;
	bForceNetUpdate = true;
}

simulated function OnPhase0SiegeEnded()
{
	local int i;
	...
	for( i = 0; i < m_TeamBlockerCache.Length; i++ )
	{
		m_TeamBlockerCache[i].DisableBlocking();
	}
}

simulated function OnResetGame()
{
	local int i;
...
simulated function Timer()
{
	...
	if (Role==ROLE_Authority && (bMatchHasBegun || bWarmupRound) && !bStopCountDown && !bMatchIsOver && Winner == None)
	{
		// game countdown...
		...
		if (TimerMessageIndex != -1)
		{
			BroadcastLocalizedMessage(class'TrTimerMessage', TimerMessageIndex);
		}

		// near win or loss...
		if (RemainingTime==TimeLimit/4)
		{
			if (Teams[0].Score>Teams[1].Score)
				BroadcastLocalizedMessage(class'TrMiscellaneousMessage',TrMISC_BloodEagleCloseToWin,none,none);
			else if (Teams[1].Score>Teams[0].Score)
				BroadcastLocalizedMessage(class'TrMiscellaneousMessage',TrMISC_DiamondSwordCloseToWin,none,none);
		}
	}
}
...
{
    // For rabbit game type, we are not a "TeamGame" yet we don't want to fail
    // the bTeamGame check in the super class, so handle this case here.
    if( GameClass == class'TrGame_TRRabbit' )
    {
        return (A.GetTeamNum() == B.GetTeamNum());
    }
    return super.OnSameTeam(A, B);
}

simulated function Blip()
{
	if( WorldInfo.NetMode != NM_DedicatedServer )
	{
		m_fCurrentSensorBlipOpacity = 1.5;
	}
...
simulated event Tick(float DeltaTime)
{
	...
	if( WorldInfo.NetMode != NM_DedicatedServer )
	{
		if( m_fCurrentSensorBlipOpacity > 0.0 )
		{
			m_fCurrentSensorBlipOpacity -= m_fSensorBlipFadeSpeed * DeltaTime;
			m_fCurrentSensorBlipOpacity = FMax(0.0,m_fCurrentSensorBlipOpacity);
		}
	}
}

simulated function float GetGlobalSpeedCapMultiplier()
{
	/*if (r_ServerConfig != none)
		return r_ServerConfig.GlobalSpeedCapMultiplier;*/

	return r_fGlobalSpeedCapMultiplier;
}

simulated function float GetGlobalAccelerationMultiplier()
{
	return r_fGlobalAccelerationMultiplier;
}

simulated function float GetGlobalAirSpeedMultiplier()
{
	return r_fGlobalAirSpeedMultiplier;
}

simulated function float GetGlobalMaxJetpackThrustSpeedMultiplier()
{
	return r_fGlobalMaxJetpackThrustSpeedMultiplier;
}

function ChangePhysicsType(int Type)
{
	r_PhysType = PhysicsType(Type);
}

simulated function bool IsClassAllowed(int TeamNum, class<TrFamilyInfo> FamilyInfo)
{
    local int i;
    local int count;
    local TrClassType classType;
    local TrPlayerReplicationInfo TrPRI;

    if (r_ServerConfig != none)
    {
        classType = FamilyInfo.default.ClassType;

        for (i = 0; i < PRIArray.Length; i++)
        {
            if (TeamNum != PRIArray[i].GetTeamNum())
            {
                continue;
            }
		    ...
		    if (IsValidTeamPlayer(TrPRI) && TrPRI.GetCurrentClass() != none && TrPRI.GetCurrentClass().default.ClassType == classType)
		    {
                count++;
		    }
        }

        return r_ServerConfig.IsClassAllowed(classType, count);
    }

    return true;
}

simulated function bool IsValidTeamPlayer(TrPlayerReplicationInfo TrPRI)
{
    if (TrPRI.bIsInactive || TrPRI.WorldInfo.NetMode != NM_Client || TrPRI.bOnlySpectator || TrPRI.GetTeamNum() == 255)
	{
		return false;
	}
...
function SetWarmingUp(bool bWarmup)
{
    local TrPlayerController TrPC;

    bWarmupRound = bWarmup;

    foreach WorldInfo.AllControllers(class'TrPlayerController', TrPC)
    {
        TrPC.bWarmingUp = bWarmup;
    }
}
...
simulated function AddPRI(PlayerReplicationInfo PRI)
{
    ...
    if (Role == ROLE_Authority)
    {
        if (!bGivePerfBonus && PRIArray.Length >= MIN_PLAYERS_FOR_PERF_BONUS)
        {
            bGivePerfBonus = true;
        }
        
        if (!bGivePrivateXP && PRIArray.Length >= MIN_PLAYERS_FOR_PRIVATE_XP)
        {
            bGivePrivateXP = true;
        }
    }
...
simulated function int GetTeamSize(byte TeamNum)
{
    local int i;
    local int count;
	...
	for (i = 0; i < PRIArray.Length; i++)
	{
	    PRI = TrPlayerReplicationInfo(PRIArray[i]);
    	
	    if ( PRI == none || PRI.bOnlySpectator || PRI.GetTeamNum() == 255)
	    {
            continue;
	    }

        if (PRI.GetTeamNum() == teamNum)
        {
            count++;
        }
	}
...
simulated function bool CanJoinTeam(byte RequestedTeam, byte CurrentTeam)
{
    local int  teamDiff;
	local bool bAllowed;

    if (RequestedTeam == 255 || RequestedTeam == CurrentTeam)
    {
        return true;
    }
    ...
    if (r_ServerConfig != none)
    {
        if (r_ServerConfig.TeamAssignType == TAT_UNBALANCED)
        {
            if (GetTeamSize(RequestedTeam) < 16)
            {
		        bAllowed = true;
            }
        }
        else if (r_ServerConfig.TeamAssignType == TAT_BALANCED || 
                 r_ServerConfig.TeamAssignType == TAT_AUTOASSIGN)
        {
            teamDiff = GetTeamSize(0) - GetTeamSize(1);
...
defaultproperties
{
   ...
   bGivePrivateXP=True
   r_bSkiEnabled=True
   r_PhysType=PhysType_AccelCap
   m_fSensorBlipFadeSpeed=0.800000
   r_fGlobalSpeedCapMultiplier=1.000000
   r_fGlobalAccelerationMultiplier=1.000000
   r_fGlobalAirSpeedMultiplier=1.000000
   r_fGlobalMaxJetpackThrustSpeedMultiplier=1.000000
   Name="Default__TrGameReplicationInfo"
