...
//
//
class GFxTrUI_ClassMenu extends GFxMoviePlayer;
...
var GFxObject InfoBoxPrimaryTF;
var GFxObject InfoBoxSecondaryTF;
var GFxObject InfoBoxPackTF;
var GFxObject InfoBoxBeltTF;
...
var array<ClassLoadout> ClassLoadouts;
...
function bool Start(optional bool StartPaused = false)
{
    ...
    AsClassOptionsComplete(TrPlayerReplicationInfo(TrPC.PlayerReplicationInfo).GetSelectedLoadout());
...
function Initialize()
{
    ...
    // Reload on the fly for new slotless system.
    TrPC.LoadPlayerProfile();

    if (InfoBoxPrimaryTF == none) InfoBoxPrimaryTF = GetVariableObject("_root.infoBox.infoText1");
    if (InfoBoxSecondaryTF == none) InfoBoxSecondaryTF = GetVariableObject("_root.infoBox.infoText2");
    if (InfoBoxPackTF == none) InfoBoxPackTF = GetVariableObject("_root.infoBox.infoText3");
    if (InfoBoxBeltTF == none) InfoBoxBeltTF = GetVariableObject("_root.infoBox.infoText4");

    GetClassLoadouts();

    for (i = 0; i < ClassLoadouts.Length; i++)
    {
        AsSetClassOption(ClassLoadouts[i].ClassType, ClassLoadouts[i].DisplayName, ClassLoadouts[i].FrameNum, ClassLoadouts[i].ArmorType); 
    }

    bInitialized = true;
}

final function AsSetClassOption(string ClassName, string DisplayName, int FrameNum, string Description)
{
    ActionScriptVoid("SetClassOption");
}

final function AsClassOptionsComplete(string CurrentSelectedClass)
{
    ActionScriptVoid("ClassOptionsComplete");
}

function string AsGetSelectedClassType()
{
	return ActionScriptString("GetClassSetting");
}

function string AsGetClassRollover()
{
	return ActionScriptString("GetClassRollover");
}

function OnClassScreenFinished()
{
    local string chosenClass;
    local class<TrFamilyInfo> FamilyInfo;
    local TrPlayerReplicationInfo PRI;

	if (TrPC == none) return;

    chosenClass = AsGetSelectedClassType();
    
    if (chosenClass == "")
    {
        TrHUD(TrPC.MyHUD).CompleteClassMenuClose();
        return;
    }
    
	FamilyInfo = class<TrFamilyInfo>(DynamicLoadObject("TribesGame."$chosenClass, class'Class'));

	// read the selected options from scaleform and make changes to the loadout
	TrPC.RequestLoadoutChange(FamilyInfo);

    TrHUD(TrPC.MyHUD).CompleteClassMenuClose();
	PRI = TrPlayerReplicationInfo(TrPC.PlayerReplicationInfo);

	// Only give our station message if we have an actual change
	if (!TrPC.IsDead() && PRI.ShouldVisitStation() && !TrPC.m_bLoadoutChangeAlerted)
	{
		// $$LOCALIZE
...
// Called via External Interface, tells us to grab the rollover class string
function OnClassRollover()
{
    ClassRollover(AsGetClassRollover());
}

function ClassRollover(string ClassType)
{
    local int i;

    for (i = 0; i < ClassLoadouts.Length; i++)
    {
        if (ClassLoadouts[i].ClassType != ClassType) continue;

        if (InfoBoxPrimaryTF != none) InfoBoxPrimaryTF.SetText(ClassLoadouts[i].Primary);
        if (InfoBoxSecondaryTF != none) InfoBoxSecondaryTF.SetText(ClassLoadouts[i].Secondary);
        if (InfoBoxPackTF != none) InfoBoxPackTF.SetText(ClassLoadouts[i].Pack);
        if (InfoBoxBeltTF != none) InfoBoxBeltTF.SetText(ClassLoadouts[i].Belt);

        return;
    }
    
    if (InfoBoxPrimaryTF != none) InfoBoxPrimaryTF.SetText("");
    if (InfoBoxSecondaryTF != none) InfoBoxSecondaryTF.SetText("");
    if (InfoBoxPackTF != none) InfoBoxPackTF.SetText("");
    if (InfoBoxBeltTF != none) InfoBoxBeltTF.SetText("");
}

function string GetArmorTypeFromClass(class<TrFamilyInfo> FamilyInfo)
{
    if (ClassIsChildOf(FamilyInfo, class'TrFamilyInfo_Light')) return "Light";
    else if (ClassIsChildOf(FamilyInfo, class'TrFamilyInfo_Heavy')) return "Heavy";
    else return "Medium";
}

function GetClassLoadouts()
{
    local int i;

	if (TrPC == none) return;

    for (i = 0; i < TrPC.m_AvailableClassIds.Length; i++)
    {
        AddClassLoadout(GetFamilyInfoFromId(TrPC.m_AvailableClassIds[i]));
    }
}

function AddClassLoadout(class<TrFamilyInfo> FamilyInfo)
{
    local int i;
	local string eqpPoint;
    local ClassLoadout loadout;
    local class<TrDevice> DeviceClass;

    if (FamilyInfo == none) return;

    loadout.ClassType = string(FamilyInfo);
    loadout.DisplayName = FamilyInfo.default.FriendlyName;
    loadout.ArmorType = GetArmorTypeFromClass(FamilyInfo);
    loadout.Enabled = true;
    loadout.FrameNum = 0;
    
	for (i = 0; i < FamilyInfo.default.DevSelectionList.Length; i++)
	{
		eqpPoint = GetSlotNameFromEquipPoint(FamilyInfo.default.DevSelectionList[i].EquipPoint);
		DeviceClass = FamilyInfo.default.DevSelectionList[i].DeviceClass;

        switch (eqpPoint)
        {
        case "Primary":
            loadout.Primary = Caps(DeviceClass.default.ItemName);
            break;
        case "Secondary":
            loadout.Secondary = Caps(DeviceClass.default.ItemName);
            break;
        case "Pack":
            loadout.Pack = Caps(DeviceClass.default.ItemName);
            break;
        case "Belt":
            loadout.Belt = Caps(DeviceClass.default.ItemName);
            break;
        default:
            break;
        }
...
defaultproperties
{
   Name="Default__GFxTrUI_ClassMenu"
