class GFxTrFront_Setup extends Object within GFxTrFront;
/* DEPRECATED
var int       OptionCount;
...
function LoadBindings(GFxObject List, array<KeyBind> Bindings)
{
	local int index;
	...
	for (index = 0; index < m_AllowedCommands.Length; index++)
	{
		...
		foreach Bindings(key)
		{
			...
			{
				AddBindOption(m_AllowedCommands[index].FriendlyName, String(key.Name));
				bFoundKey = true;
				break;
			}
		}
		...
		if (!bFoundKey)
		{
			AddBindOption(m_AllowedCommands[index].FriendlyName, "None");
		}
	}
}

function SaveBindings(GFxObject List, PlayerInput playerIn)
{
	local int i, j;
    local GFxObject elm;
...
function AddBindOption(String optName, String value)
{
    local GFxObject obj;
...
defaultproperties
{
   Name="Default__GFxTrFront_Setup"
