...
//$$JR - Flag for use in GameInfo to disallow restarting players until they're ready
var bool bPreventRespawn;
...
{
	//$$BE Make it so the cheat manager never spawns in shipping pc.
		...
		if( CurrentMove.TimeStamp <= CurrentTimeStamp )
		{
			...
			if( CurrentMove.TimeStamp == CurrentTimeStamp )
			{
					...
					&& (NewBase == CurrentMove.EndBase) )
				{
					...
					&& IsInState('PlayerWalking')
					&& ((MoveActor.Physics == PHYS_Walking) || (MoveActor.Physics == PHYS_Falling) || (MoveActor.Physics == PHYS_Skiing) || (MoveActor.Physics == PHYS_Flying)) )
				{
					CurrentMove = None;
					return;
				}
 ...
 */
//$$ TRIBESGAME ML: Added bool so we can know if a use was successful (console binding).
exec function bool Use()
{
    local bool bUsed;

	if( Role < Role_Authority )
	{
		bUsed = PerformedUseAction();
	}
	ServerUse();
    return bUsed;
}
...
function bool PerformedUseAction()
{
	...
	//$$ TRIBESGAME BE Commented out below so TriggerInteracted will be called on clients in TrPlayerController. Same code exists at the end there.
	/*if( Role < Role_Authority )
	{
		return false;
	}*/

	//$$ TRIBESGAME BE Added check for Authority.
	// leave vehicle if currently in one
	if( Role == ROLE_Authority && Vehicle(Pawn) != None )
	{
	...
	if( Role == ROLE_Authority && FindVehicleToDrive() )
	{
		return true;
	}
...
function bool IsWaiting() { return FALSE; }
...
ignores SeePlayer, HearNoise, NotifyBump, TakeDamage, PhysicsVolumeChange, NextWeapon, PrevWeapon, SwitchToBestWeapon;
	...
	function bool IsWaiting() { return TRUE; }
...
ignores SeePlayer, HearNoise, KilledBy, NotifyBump, HitWall, NotifyHeadVolumeChange, NotifyPhysicsVolumeChange, Falling, TakeDamage, Suicide;
	...
	event BeginState(Name PreviousStateName)
	{
		...
		//$$ TRIBESGAME ML: Don't show scoreboard immediately. Tribes has it on a timer.
		/*
		if( myHUD != None )
		{
