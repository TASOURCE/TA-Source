...
event PostBeginPlay()
{
	//$$ML: We do not want to clamp the upper end to 20 seconds!
	/**
	if ( MaxIdleTime > 0 )
	{
...
event PlayerController Login(string Portal, string Options, const UniqueNetID UniqueID, out string ErrorMessage)
{
	...
	// Get URL options.
	InName     = Left(ParseOption ( Options, "Name"), 21);
...
function RestartPlayer(Controller NewPlayer)
{
	...
	//$$JR - Block Respawns for 5 seconds on load and if player is in loadout menu
	if (PlayerController(NewPlayer) != None && PlayerController(NewPlayer).bPreventRespawn)
	{
		return;
	}
...
function ScoreKill(Controller Killer, Controller Other)
{
	if( (killer == Other) || (killer == None) )
	{
		//$$ML: Suicides/self kills do not decrement score.
		/*
	...
	{
		//$$BE: Friendly fire consideration.
		if (Killer.GetTeamNum() == Other.GetTeamNum())
		{
...
function bool FindInactivePRI(PlayerController PC)
{
	...
	for (i=0; i<InactivePRIArray.Length; i++)
	{
			...
			(!bIsConsole && (CurrentPRI.SavedNetworkAddress ~= NewNetworkAddress) && (CurrentPRI.PlayerName ~= NewName)) )
		{
			...
			//$$ JCR - Hook it up with the Tribes data
			//PC.PlayerReplicationInf = CurrentPRI;
            SetPRI(PC, CurrentPRI);
...
{
    PC.PlayerReplicationInfo = NewPRI;
}
...
defaultproperties
{
   ...
   DefaultGameType="TribesGame.TrGame_TRCTF"
   DefaultMapPrefixes(0)=(Prefix="TrTDM",GameType="TribesGame.TrGame_TRTDM")
   DefaultMapPrefixes(1)=(Prefix="TrCTF",GameType="TribesGame.TrGame_TRCTF")
   DefaultMapPrefixes(2)=(Prefix="TrRabbit",GameType="TribesGame.TrGame_TRRabbit")
   DefaultMapPrefixes(3)=(Prefix="TrSiege",GameType="TribesGame.TrGame_TRSiege")
   DefaultMapPrefixes(4)=(Prefix="TrTraining",GameType="TribesGame.TrGame_TrTraining")
   DefaultMapPrefixes(5)=(Prefix="TrStorm",GameType="TribesGame.TrGame_TrStorm")
   DefaultMapPrefixes(6)=(Prefix="TrTeamRabbit",GameType="TribesGame.TrGame_TrTeamRabbit")
   DefaultMapPrefixes(7)=(Prefix="TrArena",GameType="TribesGame.TrGame_TrArena")
   DefaultMapPrefixes(8)=(Prefix="TrDaD",GameType="TribesGame.TrGame_TrDaD")
   DefaultMapPrefixes(9)=(Prefix="TrCaH",GameType="TribesGame.TrGame_TrCaH")
   DefaultMapPrefixes(10)=(Prefix="TrCTFBlitz",GameType="TribesGame.TrGame_TrCTFBlitz")
