 ...
 */
//$$BE Made an event so we can access in native.
...
simulated function ProcessInstantHit(byte FiringMode, ImpactInfo Impact, optional int NumHits)
{
	...
	if (Impact.HitActor != None)
	{
		...
		//$$BE Optimization to avoid TakeDamage calls when no damage is ever applied.
		if (TotalDamage != 0)
		{
...
simulated state Active
{
	...
	//$$ML: Added this to presever the state functionality of BeginFire()
	...
	simulated function BeginFire(byte FireModeNum)
	{
		if( !bDeleteMe && Instigator != None )
		{
			...
			//$$ML: Simply moved the HasAmmo check to a new function called CanBeginFireNow so that
			// we can override it.
			//if( PendingFire(FireModeNum) && HasAmmo(FireModeNum) )
			if( PendingFire(FireModeNum) && CanBeginFireNow(FireModeNum) )
			// $$ML: End.
			{
