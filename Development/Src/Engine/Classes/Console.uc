...
//$$ TRIBESGAME PC chat coloration
struct native scrollbackData
{
    var string      Text;
...
var array<scrollbackData>   Scrollback;
 ...
 */
function OutputTextLine(coerce string Text, optional color OverrideColor) //$$ TRIBESGAME PC changed signature
{
    local scrollbackData sbd;
    ...
    sbd.Text = Text;
    sbd.OverrideColor = OverrideColor;
    ScrollBack[SBHead] = sbd;
    //ScrollBack[SBHead].Text = Text;
    //ScrollBack[SBHead].OverrideColor = OverrideColor;
    //$$ TRIBESGAME PC end
}
 ...
 */
event OutputText(coerce string Text, optional color OverrideColor)  //$$ TRIBESGAME PC changed signature
{
	...
	while(StringLength > 0)
	{
		...
		OutputTextLine(Left(RemainingText,LineLength), OverrideColor); //$$ TRIBESGAME PC
...
state Open
{
	...
	event PostRender_Console(Canvas Canvas)
	{
		...
		while (y>yl && idx>=0)
		{
			...
			Canvas.StrLen(ScrollBack[idx].Text, ScrollLineXL, ScrollLineYL); //$$ TRIBESGAME PC
			if (ScrollLineYL > yl)
			{
				y -= (ScrollLineYL - yl);
				Canvas.CurY = y;
			...
			Canvas.DrawText(Scrollback[idx].Text,false);    //$$ TRIBESGAME PC
			idx--;
			y-=yl;
		}
	}
...
defaultproperties
{
   ...
   ManualAutoCompleteList(38)=(Command="Stat STREAMING",Desc="Stat STREAMING (Displays basic texture streaming stats)")
   ManualAutoCompleteList(39)=(Command="Stat STREAMINGDETAILS",Desc="Stat STREAMINGDETAILS (Displays detailed texture streaming stats)")
   ManualAutoCompleteList(40)=(Command="Stat COLLISION",Desc="Stat COLLISION")
   ManualAutoCompleteList(41)=(Command="Stat PARTICLES",Desc="Stat PARTICLES")
   ManualAutoCompleteList(42)=(Command="Stat SCRIPT",Desc="Stat SCRIPT")
   ManualAutoCompleteList(43)=(Command="Stat AUDIO",Desc="Stat AUDIO")
   ManualAutoCompleteList(44)=(Command="Stat ANIM",Desc="Stat ANIM")
   ManualAutoCompleteList(45)=(Command="Stat NET",Desc="Stat NET")
   ManualAutoCompleteList(46)=(Command="Stat LIST",Desc="Stat LIST Groups/Sets/Group (List groups of stats, saved sets, or specific stats within a specified group)")
   ManualAutoCompleteList(47)=(Command="ListTextures",Desc="ListTextures (Lists all loaded textures and their current memory footprint)")
   ManualAutoCompleteList(48)=(Command="RestartLevel",Desc="RestartLevel (restarts the level)")
   ...
   ManualAutoCompleteList(68)=(Command="ShowDebug NET")
   ManualAutoCompleteList(69)=(Command="ShowDebug PHYSICS")
   ManualAutoCompleteList(70)=(Command="ShowDebug COLLISION")
   ManualAutoCompleteList(71)=(Command="ShowDebug AI")
   ManualAutoCompleteList(72)=(Command="ShowDebug CAMERA")
   ManualAutoCompleteList(73)=(Command="ShowDebug WEAPON")
   ManualAutoCompleteList(74)=(Command="ShowDebug ANIMATION")
   ManualAutoCompleteList(75)=(Command="ShowDebug INPUT")
   ManualAutoCompleteList(76)=(Command="memleakcheck")
