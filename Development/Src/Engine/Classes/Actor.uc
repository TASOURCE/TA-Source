...
//$$ MJD Anticheat - experiment to move this somewhere else to see if it foobars the aimbots
...
//$$ MJD June 2011 - Performance
var const transient int     IndexInTickList;
var const transient float   LastSlowRelevancyCheckTime;
var                 bool    s_bThrottleNetRelevancy;      // whether to throttle relevancy evaluation via IsNetRelevant
//$$ end MJD
//$$ MJD AntiCheat - moved from above to confuse cheaters
var(Movement) const vector			Location;
...
var(Collision) const transient enum ECollisionType
{
	...
	COLLIDE_BlockVehicles, //$$BE only blocks vehicles
...
var bool m_bPotentialSeekingTarget;
//$$ML: End.
...
//$$BE
/* VisibleCollidingExtentActors() returns all colliding (bCollideActors==true) actors within a certain radius
for which a trace from Loc (which defaults to caller's Location) to that actor's Location (BBox center and BBox top) does not hit the world.
Much faster than AllActors() since it uses the collision octree
bUseOverlapCheck uses a sphere vs. box check instead of checking to see if the center of an object lies within a sphere
*/
native final iterator function VisibleCollidingExtentActors(class<actor> BaseClass, out actor Actor, float Radius, optional vector Loc, optional vector AltLoc, optional bool bIgnoreHidden, optional vector Extent, optional bool bTraceActors, optional class<Interface> InterfaceClass, optional out TraceHitInfo HitInfo, optional float XYCheckRadius);
...
simulated function bool EffectIsRelevant(vector SpawnLocation, bool bForceDedicated, optional float VisibleCullDistance=5000.0, optional float HiddenCullDistance=350.0 )
{
	...
	{
		if(bDebugEffectIsRelevant && ( WorldInfo.TimeSeconds - LastRenderTime < 0.3 )) { DebugMessagePlayer("EffectIsRelevant FAILED - SpawnLocation == Location"); };
...
//$$
simulated function string GetSpectatorName(){return "";}
simulated function string GetSpectatorDescription(){return "";}
simulated function GetSpectatorHealthInfo(out int Health, out int MaxHealth){Health=-1;MaxHealth=-1;}
...
cpptext
{
	...
	//$$BE RPC Injection Protection
	//UBOOL ProcessRemoteFunction( UFunction* Function, void* Parms, FFrame* Stack );
	UBOOL ProcessRemoteFunction( UFunction* Function, void* Parms, FFrame* Stack, UBOOL bSkipStack = FALSE );
	//$$BE End RPC Injection Protection
...
defaultproperties
{
   ...
   TickFrequencyLastSeenTimeBeforeForcingMaxTickFrequency=2.000000
