...
struct native LastHitInfoStruct
{
    var bool  bDirectHit;
	var float Amount;
	var float Distance;
...
var LastHitInfoStruct LastHitInfo;
...
event TakeDamage(int Damage, Controller InstigatedBy, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo HitInfo, optional Actor DamageCauser)
{
	...
	if ( Health <= 0 )
	{
		...
		//$$ML: Also call take hit when pawn dies.
...
//$$ML: Added check to see if we should damage the pawn we just ran into.
event bool CheckClotheslineDamage(Pawn PawnHittingMe) {return false;}
//$$ML: End.
...
cpptext
{
	...
	//$$ML: Be able to adjust WalkableFloorZ based on the type of surface we're walking on.
	virtual float GetWalkableFloorZ(class AActor* TestBase=NULL){ return WalkableFloorZ; }
