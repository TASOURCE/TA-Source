...
native final function bool CreateViewportConsole(class vpcClass, out string OutError); //$$ PC CONSOLE_LOCKDOWN (moved to native so Release mode will launch one even when using Shipping-PC cooked scripts)
/**
...
event bool Init(out string OutError)
{
    ...
    //$$ PC CONSOLE_LOCKDOWN
    if(CreateViewportConsole(Outer.ConsoleClass, OutError) == false)
    {
        return false;
    }
    //$$ endif
...
event PostRender(Canvas Canvas)
{
    ...
    if(ViewportConsole != None) //$$ PC CONSOLE_LOCKDOWN
    {
	    // Render the console.
